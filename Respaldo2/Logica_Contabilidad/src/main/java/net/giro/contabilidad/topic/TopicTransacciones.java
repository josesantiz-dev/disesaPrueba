...
import net.giro.contabilidad.logica.ConceptosFac;
...
public class TopicTransacciones implements MessageListener {
	...
	LinkedHashMap<Long, List<Long>> mapEquivConceptos;
	...
	private boolean procesaTransaccion(String mensaje) {
		...
		List<OperacionesIntegradasConceptosSql> listaConceptosSQL = null;
		...
		try {
        	...
        	this.mapEquivConceptos = this.equivalenciaConceptos(this.msgTransaccion.getIdTransaccion());
			
			// Recuperamos Campos  **** NOTA..con esta consulta estás obteniendo los campos no los conceptos 
			log.info("Recuperando Campos y Conceptos ...");
        	...
        	// -------------------------------------------------------------------------------
			log.info("Generando Poliza y Lote ...");
			...
			log.info("Seperando Campos y Conceptos ...");
			...
			for (OperacionesIntegradasSql iConcepto : listOISQLConceptos) {
				...
				log.info("Verificando Equivalencia de concepto. Actual: " + idConcepto);
				idConcepto = matchConcepto(idConcepto);
				log.info("Verificando Equivalencia de concepto. Nuevo: " + idConcepto);
				...
				log.info("Recuperando OperacionesIntegradasConceptosSQL con el Concepto: " + idConcepto + " - " + conceptoActual);
				listaConceptosSQL = this.ifzOIConceptosSQL.findByProperties(params, 0);
				...
				column = 0;
				log.info(listaConceptosSQL.size() + " OperacionesIntegradasConceptosSQL encontradas");
				for (OperacionesIntegradasConceptosSql OIConceptoSql : listaConceptosSQL) {
					...
					while (rs.next()) {
						...
						if (rs.getMetaData().getColumnCount() >= 3) {
							boolean llavesOk = false;
							for (int col = 3; col <= rs.getMetaData().getColumnCount(); col++) {
								if (! rs.getMetaData().getColumnName(col).toLowerCase().trim().contains("llave")) {
									if (rs.getString(col) != null && ! "".equals(rs.getString(col).trim()))
										aux.put(rs.getMetaData().getColumnName(col), rs.getString(col));
									continue;
								}
    ...
    private LinkedHashMap<Long, List<Long>> equivalenciaConceptos(long codigoTransaccion) {
    	LinkedHashMap<Long, List<Long>> mapConceptos = null;
    	...
    	try {
    		if (codigoTransaccion <= 0L) 
    			return mapConceptos;
    		mapConceptos = new LinkedHashMap<Long, List<Long>>();
			
    		// Generamos consulta y Ejecutamos consulta
        	strQuery = "SELECT id_concepto_origen, id_concepto_destino FROM equivalencia_conceptos_contables WHERE codigo_transaccion = " + codigoTransaccion + " ORDER BY codigo_transaccion, id_concepto_origen, id_concepto_destino ";
        	rs = this.conn.createStatement().executeQuery(strQuery);
    		while (rs.next()) {
			...
			log.info(mapConceptos.size() + " Equivalencias de impuestos encontradas para la transaccion " + codigoTransaccion);
    	}
    	
    	return mapConceptos;
    }
    
    private long matchConcepto(long idConcepto) {
    	long idConceptoMatch = 0L;
    	
    	if (this.mapEquivConceptos == null || this.mapEquivConceptos.isEmpty())
    		return idConcepto;
    	
    	idConceptoMatch = idConcepto;
    	for (Entry<Long, List<Long>> item : this.mapEquivConceptos.entrySet()) {
    		if (idConcepto == item.getKey().longValue()) {
    			idConceptoMatch = item.getValue().get(0);
    			break;
    		}
    	}
    	
    	return idConceptoMatch;
    }
	...
	private long getConceptoId(String concepto) throws Exception {
		...
		if (concepto == null || "".equals(concepto.trim()) || (! concepto.contains("#")))
			return idConcepto;

		log.info("---> Buscando ID Concepto [" + concepto + "] ... ");
		concepto = concepto.trim().replace("#", "");
