...
public class ConvertExt {
	...
	public PagosGastosExt PagosGastosToPagosGastosExt(PagosGastos pojoTarget){
		...
		try {
			...
			if (pojoTarget.getIdSucursal() != null && pojoTarget.getIdSucursal() > 0L){
				...
				pojoResult.setIdSucursal(pojoAux);//(SucursalToSucursalExt(pojoAux));
			}
	...
	public PagosGastosDet PagosGastosDetExtToPagosGastosDet(PagosGastosDetExt pojoTarget) throws Exception {
		...
		try {
			...
			if (pojoTarget.getIdProveedor() != null)
	...
	public PagosGastosDetExt PagosGastosDetToPagosGastosDetExt(PagosGastosDet pojoTarget) throws Exception {
		...
		try {
			...
			pojoResult.setObservaciones(pojoTarget.getObservaciones());
			pojoResult.setReferencia(pojoTarget.getReferencia());
			pojoResult.setFacturaRazonSocial(pojoTarget.getFacturaRazonSocial());
			pojoResult.setSubtotal(pojoTarget.getSubtotal());
			pojoResult.setTotalImpuestos(pojoTarget.getTotalImpuestos());
			pojoResult.setTotalRetenciones(pojoTarget.getTotalRetenciones());
			pojoResult.setFacturado(pojoTarget.getFacturado());
			...
			if (pojoTarget.getIdProveedor() != 0L) {
				...
				if ("N".equals(pojoTarget.getTipoPersonaProveedor())) {
					Negocio pojoNegocio = ifzNegocios.findById(pojoTarget.getIdProveedor());
					if (pojoNegocio == null)
						pojoNegocio = new Negocio();
					pojoPersona.setId(pojoNegocio.getId());
					pojoPersona.setNombre(pojoNegocio.getNombre());
			...
			if (pojoTarget.getIdXml() != null && pojoTarget.getIdXml() > 0L) {
				pojoResult.setFacturaUuid(this.ifzPagosGastosDet.getFacturaProperty(pojoTarget.getIdXml(), "uuid"));
				pojoResult.setFacturaSerie(this.ifzPagosGastosDet.getFacturaProperty(pojoTarget.getIdXml(), "serie"));
				pojoResult.setFacturaFolio(this.ifzPagosGastosDet.getFacturaProperty(pojoTarget.getIdXml(), "folio"));
				pojoResult.setFacturaRfc(this.ifzPagosGastosDet.getFacturaProperty(pojoTarget.getIdXml(), "rfcEmisor"));
				pojoResult.setFacturaRazonSocial(this.ifzPagosGastosDet.getFacturaProperty(pojoTarget.getIdXml(), "nombre"));
				pojoResult.setFacturaTotal(this.ifzPagosGastosDet.getFacturaProperty(pojoTarget.getIdXml(), "total"));
			}
		} catch (Exception e) {
