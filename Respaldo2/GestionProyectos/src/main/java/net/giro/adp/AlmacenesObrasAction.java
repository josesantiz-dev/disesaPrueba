...
public class AlmacenesObrasAction implements Serializable {
	...
	public AlmacenesObrasAction() {
		...
		try {
			...
			this.busquedaCampo = this.busquedaOpciones.get(0).getDescription();
			this.busquedaValor = "";
			this.busquedaTipo = 0;
    ...
    private void cargaAlmacenes() {
    	...
    	try {
    		...
    		listaAlmacenesFull = this.ifzAlmacenes.findAllActivos();
    ...
    public boolean comprobarAlmacenPrincipalDeObra() {
    	if (this.almacenPrincipal) {
        	for (ObraAlmacenes item : this.listObraAlmacenes) {
    			if (item.getIdAlmacen().longValue() != this.pojoAlmacen.getId().longValue() && item.getAlmacenPrincipal() == 1) {
    ...
    public boolean validarAlmacenObra() {
    	try {
			...
			control(true, -1, "Ocurrio un problema al validar los datos para añadir a la Obra el Almacen seleccionado", e);
    		return false;
    	}
    }
