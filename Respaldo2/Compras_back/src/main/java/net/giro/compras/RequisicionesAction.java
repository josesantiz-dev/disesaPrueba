...
public class RequisicionesAction implements Serializable {
	...
	public RequisicionesAction() {
		...
		try {
			...
			this.tiposBusqueda.add(new SelectItem("id", "ID Requisicion"));
	...
	public void buscar() throws Exception {
		try {
			...
			//this.ifzRequisiciones.OrderBy("model.sistema, CASE model.estatus WHEN 0 THEN 1 WHEN 2 THEN 2 WHEN 1 THEN 3 ELSE 4 END, model.id desc");
	...
	public void guardar() throws Exception {
		...
		try {
			...
			if (this.pojoRequisicion.getId() == null) {
				...
				this.listRequisiciones.add(0, this.ifzRequisiciones.findById(this.pojoRequisicion.getId()));
			...
			if (this.listReqDetallesEliminados != null && !this.listReqDetallesEliminados.isEmpty()) {
				if (this.isDebug) 
					log.info("Borrando detalles de Requisicion previamente eliminados.");
				...
				this.listReqDetallesEliminados.clear();
				log.info("Detalles de Requisicion borrados");
			}
	...
	public void eliminar() {
		try {
    		control();
    		// Comprobamos si podemos eliminar Requisicion
    		if (! comprobarEliminar()) 
    			return;

			// Actualizamos el elemento de la lista
    		log.info("Cancelando Requisicion ... ");
			for (Requisicion var : this.listRequisiciones) {
				if (var.getId() == this.pojoRequisicionBorrar.getId()) {
					if (this.isDebug) 
						log.info("Borrando Requisicion.");
					var.setEstatus(1);
					var.setModificadoPor(this.usuarioId);
					var.setFechaModificacion(Calendar.getInstance().getTime());
	...
	public void reporte() {
		...
		try{
			if (this.pojoRequisicionReporte != null && this.pojoRequisicionReporte.getId() != null && this.pojoRequisicionReporte.getId() > 0L) {
				...
				log.info("REPORTE REQUISICION: Ejecutamos reporte");
				...
				// Recogemos reporte
				log.info("REPORTE REQUISICION: Recuperamos contenido de reporte");
				...
				nombreDocumento = "REQ-" + this.pojoRequisicionReporte.getId() + "." + formatoDocumento;
				if (contenidoDocumento == null || contenidoDocumento.length <= 0) {
					log.error("Error Interno - No se pudo obtener el reporte de cotizacion ID " + this.pojoRequisicionReporte.getId());
		    		control(-1, "Error Interno - No se pudo recuperar el reporte de Cotizacion " + this.pojoRequisicionReporte.getId());
					return;
				}

				log.info("REPORTE REQUISICION: Lo guardo en SESSION");
				...
				log.info("REPORTE REQUISICION: Terminado");
			...
			control("Ocurrio un problema al intentar exportar la Requisicion", e);
		}
	}
	...
	private void cargarSolicitantes() {
		...
		try {
			...
			listEmpleados = this.ifzEmpleados.findAllActivos();
	...
	private boolean comprobarEliminar() {
		List<Cotizacion> listTmp = null;
		String cotizaciones = "";
		
		try {
			if (this.pojoRequisicionBorrar == null || this.pojoRequisicionBorrar.getId() == null || this.pojoRequisicionBorrar.getId() <= 0L)
				return true;
			
			listTmp = this.ifzCotizaciones.findByProperty("idRequisicion", this.pojoRequisicionBorrar.getId(), 0);
			if (listTmp != null && ! listTmp.isEmpty()) {
				for (Cotizacion item : listTmp) {
					if (! "".equals(cotizaciones))
						cotizaciones += ", ";
					cotizaciones += item.getFolio();
				}
			}
			
			if (! "".equals(cotizaciones)) {
				control(-1, "No se puede Cancelar la Requisicion.\n Esta siendo usada en la(s) Cotizacion(es):\n" + cotizaciones);
				return false;
			}
		} catch (Exception e) {
			control("Ocurrio un problema al intentar Cancelar la Requisicion", e);
			return false;
		}
	...
	private void control(boolean operacionCancelada, int tipoMensaje, String mensaje, Throwable throwable) {
		...
		log.error("\n\n REQUISICIONES :: " + this.usuario + " :: " + codigo + "+" + this.tipoMensaje + "\n" + mensaje + "\n" + this.mensajeDetalles, throwable);
	}
	...
	private void copiaActualizaEnGastos(Producto pojoProducto) {
		...
		try {
			...
			lista = this.ifzConValores.buscaValorGrupo("valor", String.valueOf(pojoProducto.getId()), this.pojoGpoGastos); //.findByPropertyPojoSolito("valor", String.valueOf(pojoProducto.getId()));
			if (lista != null && ! lista.isEmpty())
				pojoGasto = lista.get(0);
