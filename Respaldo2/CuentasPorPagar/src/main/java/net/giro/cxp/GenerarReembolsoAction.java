...
public class GenerarReembolsoAction implements Serializable {
			...
			if ("3".equals(this.operacion)){
				...
				if (pojoCtasBanco != null && pojoValores != null){
					...
					if ("".equals(this.mensaje)){
						...
						this.pojoMovCtas = this.ifzMovCta.actualizar(pojoMovCtas, "G",Calendar.getInstance().getTime());
				...
				if ("".equals(this.mensaje)){
					...
					this.pojoMovCtas = ifzMovCta.actualizar(pojoMovCtas, "G",Calendar.getInstance().getTime());
					this.listMovCtas.remove(pojoMovCtas);
					this.muestraSalvar = false;
				}
			}
	...
	public String cancelarCheques(){
   		try{
   			...
   			resp = ifzMovCta.salvar(this.pojoMovCtas, false);
	...
	public CuentasBancariasRem getIfzCtas() {
		return ifzCtas;
	}

	public void setIfzCtas(CuentasBancariasRem ifzCtas) {
