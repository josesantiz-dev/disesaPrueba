...
public class GastosComprobarAction {
	...
	public GastosComprobarAction() throws NamingException, Exception{
		...
		//this.ifzQuery.setInfoSesion(this.loginManager.getInfoSesion());
		...
		//this.ifzSpei.setInfoSesion(this.loginManager.getInfoSesion());
	...
	public String guardar() {
		try{
			...
			if( registroNuevo ){
				...
				// Guardamos
				resp = this.ifzGtosComp.salvar(this.pojoGtosComp, true); // resp = this.ifzGtosComp.salvar(this.pojoGtosComp, true, null);
	...
	public void mensajeTransaccion() {
		try {
			...
			Respuesta respuesta = this.ifzGtosComp.enviarTransaccion(this.pojoGastoComprobar);
	...
	public String cancelarCheques(){
		try{
			Respuesta resp = new Respuesta();
			resp = ifzGtosComp.salvar(this.pojoGtosComp, false);
	...
	public long getSucursal() {
		if (this.pojoGtosComp != null && this.pojoGtosComp.getIdSucursal() != null)
			return this.pojoGtosComp.getIdSucursal().getId();
		return 0L;
	}

	public void setSucursal(long idSucursal) {
		if (idSucursal > 0L && this.pojoGtosComp != null) {
			for (Sucursal var : this.listSucursales) {
