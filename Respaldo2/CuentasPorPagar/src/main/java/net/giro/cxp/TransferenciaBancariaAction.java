...
public class TransferenciaBancariaAction {
	...
	private CuentasBancariasRem ifzBancos;
	...
	private List<CuentaBancaria> listBancos;
	...
	public TransferenciaBancariaAction() throws NamingException,Exception {
		...
		this.usuarioId = this.loginManager.getUsuarioResponsabilidad().getUsuario().getId(); //lm.getLoginBean().getUsuario().getUsuarioId();
		...
		this.ifzBancos 		= (CuentasBancariasRem) this.ctx.lookup("ejb:/Logica_TYG//CuentasBancariasFac!net.giro.tyg.logica.CuentasBancariasRem");
		...
		this.ifzTipoCambio.setInfoSesion(this.loginManager.getInfoSesion());
	...
	public String cancelaTrans (){
		...
		try{
			resp = ifzMovCta.cancelacion(pojoMovCtas, Calendar.getInstance().getTime());
		...
		try{
			...
			if ("".equals(this.msgError)){
				...
				if ("".equals(msgError)){
					...
					resp = ifzMovCta.salvar(pojoMovCtas, true);
					if (resp.getResultado() == 0){
	...
	public List<CuentaBancaria> getListBancos() {
		return listBancos;
	}

	public void setListBancos(List<CuentaBancaria> listBancos) {
