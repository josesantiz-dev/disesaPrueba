...
public abstract class DAOImpl<T extends Serializable> implements DAO<T> {
    ...
    public long save(T entity, Long idEmpresa) throws ExcepConstraint {
		...
		try {
			...
			log.error("GP :: Error REFLECTION ", e);
		...
		try {
			...
			log.error("GP :: Error ASIGNAR SECUENCIA :: " + tabla + " :: " + secuen, e);
		...
		try {
			...
			log.error("GP :: Error GRABAR ENTIDAD :: " + tabla + " :: " + secuen, e);
		...
		for (T entity : entities) {
			...
			if (idStored <= 0) {
				try {
					...
					log.error("GP :: Error ASIGNAR SECUENCIA :: " + tabla + " :: " + secuen + " [" + index + "]", e);
					throw new ExcepConstraint("ASIGNAR SECUENCIA", entity);			
				}
				...
				try {
					...
					log.error("GP :: Error GRABAR ENTIDAD :: " + tabla + " :: " + secuen + " [" + index + "]", e);
					throw new ExcepConstraint("GRABAR ENTIDAD", entity);			
				}
			} else {
				// Actualizo
				try {
					...
					log.error("GP :: Error ACTUALIZAR ENTIDAD :: " + tabla + " :: " + idStored + " [" + index + "]", e);
...
//----------------------------------------------------------------------------------------------------------------'
//2.2		|19/05/2016		|Javier Tirado	| Renombramos la clase de DAOImplFact a DAOImpl