...
public class CorrelativoSession implements CorrelativoInterfaceLocal {
	...
	public List<Correlativo> findByProperty(String propertyName, Object value) {
		try {
			String queryString = "select model from Correlativo model where model." + propertyName + "= :propertyValue";
			return entityManager.createQuery(queryString).setParameter("propertyValue", value).getResultList();
		} catch (RuntimeException re) {
			re.printStackTrace();
			throw re;
		}
	...
	@TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
	private String getSequencia(Long codigoEmpresa, String nombreTabla) {
