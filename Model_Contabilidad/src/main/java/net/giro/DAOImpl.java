...
public abstract class DAOImpl<T extends Serializable> implements DAO<T> {
    ...
    public long save(T entity, Long idEmpresa) throws ExcepConstraint {
		...
		try {
			...
			log.error("CONTABILIDAD :: Error REFLECTION ", e);
		...
		try {
			...
			log.error("CONTABILIDAD :: Error ASIGNAR SECUENCIA :: " + tabla + " :: " + secuen, e);
		...
		try {
			...
			log.error("CONTABILIDAD :: Error GRABAR ENTIDAD :: " + tabla + " :: " + secuen, e);
	...
	public List<T> saveOrUpdateList(List<T> entities, Long idEmpresa) throws Exception {
		...
		for (T entity : entities) {
			...
			if (idStored <= 0) {
				try {
					// Genero ID
					secuen = this.ifzCorrelativo.GenerarCorrelativo(idEmpresa, tabla);
					if (secuen.longValue() <= 0L)
						continue;
					setter.invoke(entity, secuen);
				} catch (Exception e) {
					log.error("CONTABILIDAD :: Error ASIGNAR SECUENCIA :: " + tabla + " :: " + secuen + " [" + index + "]", e);
					throw new ExcepConstraint("ASIGNAR SECUENCIA", entity);
				}
				
				try {
					base = this.findById(secuen);
					if (base != null)
						throw new Exception("ID para " + tabla + " duplicado");
				} catch (Exception e) {
					log.error("CONTABILIDAD :: Error SECUENCIA REPETIDA :: " + tabla + " :: " + secuen + " [" + index + "]", e);
					throw new ExcepConstraint("SECUENCIA REPETIDA", entity);
				}
				...
				try {
					...
					log.error("CONTABILIDAD :: Error GRABAR ENTIDAD :: " + tabla + " :: " + secuen + " [" + index + "]", e);
					throw new ExcepConstraint("GRABAR ENTIDAD", entity);
				}
			} else {
				// Actualizo
				try {
					...
					log.error("CONTABILIDAD :: Error ACTUALIZAR ENTIDAD :: " + tabla + " :: " + idStored + " [" + index + "]", e);
