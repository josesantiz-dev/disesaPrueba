...
public class ConValoresImp extends DAOImpl<ConValores> implements ConValoresDAO  {
	...
	/*@Override
	public void delete(ConValores entity) throws Exception {
		try {
	...
	public List<ConValores> findAll(ConGrupoValores grupo, String orderBy, int limite) {
		String queryString = "select model from ConValores model where model.id not in (select x.idConValores.id from ConValoresBlacklist x where x.idGrupo.id = :idGrupo) ";
		
		try {
			queryString += "and model.grupoValorId.id = :idGrupo ";
	...
	public ConValores findByPropertyPojoSolito(String propertyName,final Object value) {
		try {
			final String queryString = "select model from ConValores model where model." + propertyName + "= :propertyValue";
			Query query = entityManager.createQuery(queryString);
	...
	public ConValores findByValorGrupo(String valor, ConGrupoValores grupo) {
		...
		try {
			queryString += "and trim(lower(model.valor)) = trim('" + valor + "') and model.grupoValorId.id = :idGrupo order by model.descripcion";
	...
	public List<ConValores> buscaValorGrupo(String campo, String valor, ConGrupoValores grupo) {
		...
		try {
			if ("valorId".equals(campo)) 
				queryString = queryString + "and lower(cast(model.id, string)) like '%"+(valor != null ? valor.toLowerCase() : "") + "%' ";
			else 
				queryString = queryString + "and lower(model."+ campo +") like '%"+(valor != null ? valor.toLowerCase() : "") + "%' ";
			queryString = queryString + " and model.grupoValorId.id = :idGrupo order by model.descripcion";
	...
	public List<ConValores> findLikeValorIdPropiedadGrupo(String dato, ConGrupoValores grupo,int limit){
		try {
			...
			final String queryString = "select conVal from ConValores conVal where model.id not in (select x.idConValores.id from ConValoresBlacklist x where x.idGrupo.id = :idGrupo) " +																			
						"and (lower(conVal.descripcion) like '%" + dato.toLowerCase() + "%' or cast(conVal.id, string) like '%" + dato.toLowerCase() + "%' ) " +
						"and conVal.grupoValorId.id = :idGrupo order by conVal.descripcion ";
			Query query = entityManager.createQuery(queryString);
			query.setParameter("idGrupo", grupo.getId());
	...
	public List<ConValores> findByProperty(String propertyName, Object value, ConGrupoValores grupo, int limit) throws Exception {
		...
		try {
			...
			queryString += " and model.grupoValorId.id = :idGrupo order by model.descripcion";
	...
	public List<ConValores> findLikeProperty(String propertyName, String value, ConGrupoValores grupo, String tipoCuenta, int limit) throws Exception {
		...
		try {
			if ("valorId".equals(propertyName))
				queryString += "and lower(cast(model.id, string)) like '%" + (value != null ? value.toLowerCase() : "") + "%' ";
			else
				queryString += "and lower(model."+ propertyName +") like '%" + (value != null ? value.toLowerCase() : "") + "%' ";
			if (tipoCuenta != null && ! "".equals(tipoCuenta.trim()))
				queryString += "and lower(model.tipoCuenta) = :tipoCuenta ";
			queryString += "and model.grupoValorId.id = :idGrupo order by model.descripcion";
			
			Query query = entityManager.createQuery(queryString);
			query.setParameter("idGrupo", grupo.getId());
			if (tipoCuenta != null && ! "".equals(tipoCuenta.trim()))
				query.setParameter("tipoCuenta", tipoCuenta.trim().toLowerCase());
			if (limit > 0)
				query.setMaxResults(limit);
			return query.getResultList();
		} catch (Exception re) {
