...
public abstract class DAOImpl<T extends Serializable> implements DAO<T> {
    ...
    @EJB
    private CorrelativoInterfaceLocal ifzCorrelativo;
    protected Class<T> persistenceClass;
    protected int limitResult;
    
	public DAOImpl() {
        this.persistenceClass = (Class<T>) ((ParameterizedType) getClass().getGenericSuperclass()).getActualTypeArguments()[0];
        this.limitResult = 1000;
    }
    ...
    public long save(T entity, Long idEmpresa) throws ExcepConstraint {
		...
		try {
			...
			log.error("PUBLICO :: Error REFLECTION ", e);
		...
		try {
			...
			log.error("PUBLICO :: Error ASIGNAR SECUENCIA :: " + tabla + " :: " + secuen, e);
		...
		try {
			...
			log.error("PUBLICO :: Error GRABAR ENTIDAD :: " + tabla + " :: " + secuen, e);
		...
		for (T entity : entities) {
			...
			if (idStored <= 0) {
				try {
					...
					log.error("PUBLICO :: Error ASIGNAR SECUENCIA :: " + tabla + " :: " + secuen + " [" + index + "]", e);
					throw new ExcepConstraint("ASIGNAR SECUENCIA", entity);			
				}
				...
				try {
					...
					log.error("PUBLICO :: Error GRABAR ENTIDAD :: " + tabla + " :: " + secuen + " [" + index + "]", e);
					throw new ExcepConstraint("GRABAR ENTIDAD", entity);			
				}
			} else {
				// Actualizo
				try {
					...
					log.error("PUBLICO :: Error ACTUALIZAR ENTIDAD :: " + tabla + " :: " + idStored + " [" + index + "]", e);
    ...
    public List<T> findLikeColumnName(String columnName, String value) {
    	...
    	if (value != null && ! "".equals(value)) {
    		sqlWhere += " WHERE lower(o." + columnName + ") LIKE :value ";
    		sb = new StringBuffer();
    		sb.append("%");
