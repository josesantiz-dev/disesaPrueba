...
public class Navegador implements Serializable {
    ...
    public void inicializa() {
    	this.numPag = 1;
    	this.miPag = new Pagina();
    	this.menuRoot = null;
    	this.mensaje = "Ocurrio un problema, no se pudo mostrar la pagina solicitada!";
    }
    ...
    public boolean sessionAbierta() {
        return this.httpSession.getAttribute("Logeado") != null && Boolean.valueOf(this.httpSession.getAttribute("Logeado").toString());
    }
    ...
    public String cerrarSession() { 
		...
		try {
			facesContext = FacesContext.getCurrentInstance();
			ec = facesContext.getExternalContext();
			ec.redirect("/cas/logout?service=https://disesa.giro26.net:9443/DISESA/index.faces");
			this.httpSession = (HttpSession) ec.getSession(false);
			this.httpSession.invalidate();
		...
		try {
			if (this.menuRoot == null) { 
				if (this.mediator.getLoginManager().isLogeado()) {
					menu = this.mediator.getLoginManager().getInfoSesion().getResponsabilidad().getResponsabilidad().getMenu();
					this.menuRoot = this.mediator.getLoginManager().getAutentificacion().getMenu(menu);
				}
			}
		} catch (Exception e) {
			log.error("Error en metodo getMenu",e);
		}
		...
		try {
			...
			for (Pagina p : root.getSubMenu()) {
				if (p.getSubMenu() != null) {
					...
					grupo.setId("item" + p.getId() + this.id++);
					construyeArbol(grupo, p);
					...
					item.setId("item" + p.getId() + this.id++);
