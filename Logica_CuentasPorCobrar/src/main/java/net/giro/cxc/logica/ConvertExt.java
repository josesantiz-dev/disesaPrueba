...
public class ConvertExt implements Serializable {
	...
	public ConvertExt() {
		Hashtable<String, Object> envorinment = null;
		
		try {
			envorinment = new Hashtable<String, Object>();
            envorinment.put(Context.URL_PKG_PREFIXES, "org.jboss.ejb.client.naming");
    		this.ctx = new InitialContext(envorinment);
    		...
    		this.ifzTimbresPagos = (FacturaPagosTimbreRem) this.ctx.lookup("ejb:/Logica_CuentasPorCobrar//FacturaPagosTimbreFac!net.giro.cxc.logica.FacturaPagosTimbreRem");
	...
	public Factura FacturaExtToFactura(FacturaExt pojoTarget) {
		...
		try {
			...
			pojoResult.setMetodoPago(pojoTarget.getMetodoPago());
			...
			pojoResult.setTimbradoPor(pojoTarget.getTimbradoPor());
			...
			pojoResult.setIdFacturaRelacionada(0L);
			...
			if (this.mostrarSystemOut) log.info("---- -- ----> Comprobando FacturaRelacionada");
			if (pojoTarget.getIdFacturaRelacionada() != null && pojoTarget.getIdFacturaRelacionada().getId() != null && pojoTarget.getIdFacturaRelacionada().getId() > 0) {
				pojoResult.setIdFacturaRelacionada(pojoTarget.getIdFacturaRelacionada().getId());
				if (this.mostrarSystemOut) log.info("---- -- ----> Comprobando FacturaRelacionada terminado");
			}

			if (this.mostrarSystemOut) log.info("---- -- ----> Comprobando IdTimbre");
			if (pojoTarget.getIdTimbre() != null && pojoTarget.getIdTimbre().getId() != null && pojoTarget.getIdTimbre().getId() > 0L) {
				pojoResult.setIdTimbre(pojoTarget.getIdTimbre().getId());
			}
		} catch (Exception e) {
			throw e;
		}
	...
	public FacturaExt FacturaToFacturaExt(Factura pojoTarget) throws Exception {
		...
		try {
			...
			if (this.mostrarSystemOut) log.info("---- -- ----> Comprobando FacturaRelacionada");
			if (pojoTarget.getIdFacturaRelacionada() > 0L) {
				Factura pojoAux = this.ifzFacturas.findById(pojoTarget.getIdFacturaRelacionada());
				if (pojoAux == null) 
					pojoAux = new Factura();
				pojoResult.setIdFacturaRelacionada(pojoAux);
				if (this.mostrarSystemOut) log.info("---- -- ----> Comprobando FacturaRelacionada terminado");
			}

			if (this.mostrarSystemOut) log.info("---- -- ----> Comprobando BancoDeposito");
			if (pojoTarget.getIdTimbre() > 0) {
				FacturaTimbre pojoTimbre = this.ifzTimbres.findById(pojoTarget.getIdTimbre());
				pojoResult.setIdTimbre(pojoTimbre);
			}
		} catch (Exception e) {
			throw e;
		}
	...
	public FacturaDetalleExt FacturaDetalleToFacturaDetalleExt(FacturaDetalle pojoTarget) throws Exception{
		...
		try {
			...
			if (pojoTarget.getIdUnidadMedida() > 0L) {
	...
	public FacturaDetalleImpuesto FacturaDetalleImpuestoExtToFacturaDetalleImpuesto(FacturaDetalleImpuestoExt pojoTarget){
		...
		try {
			if (pojoTarget == null)
				return null;
			pojoResult.setId(pojoTarget.getId());
			pojoResult.setTasa(pojoTarget.getTasa());
			pojoResult.setTipoFactor(pojoTarget.getTipoFactor());
	...
	public FacturaDetalleImpuestoExt FacturaDetalleImpuestoToFacturaDetalleImpuestoExt(FacturaDetalleImpuesto pojoTarget) throws Exception{
		...
		try {
			...
			if (pojoTarget.getIdImpuesto() > 0L) {
				...
				if (pojoAux == null) pojoAux = new ConValores();
	...
	public FacturaPagos FacturaPagosExtToFacturaPagos(FacturaPagosExt pojoTarget) {
		...
		try {
			...
			pojoResult.setPagoRecibido(pojoTarget.getPagoRecibido());
			pojoResult.setDiferencia(pojoTarget.getDiferencia());
			pojoResult.setPago(pojoTarget.getPago());
			//pojoResult.setSaldoAnterior(pojoTarget.getSaldoAnterior()); // ASIGNADO AL FINAL
			//pojoResult.setSaldoInsoluto(pojoTarget.getSaldoInsoluto()); // ASIGNADO AL FINAL
			pojoResult.setObservaciones(pojoTarget.getObservaciones());
			pojoResult.setTimbrado(pojoTarget.getTimbrado());
			pojoResult.setTimbradoPor(pojoTarget.getTimbradoPor());
			pojoResult.setUuid(pojoTarget.getUuid());
			pojoResult.setSerie(pojoTarget.getSerie());
			pojoResult.setFolio(pojoTarget.getFolio());
			pojoResult.setAgrupador(pojoTarget.getAgrupador());
			...
			if (pojoTarget.getIdMoneda() != null && pojoTarget.getIdMoneda().getId() > 0) {
				...
				pojoResult.setMoneda(pojoTarget.getIdMoneda().getNombre());
			}
			...
			if (pojoTarget.getIdFormaPago() != null && pojoTarget.getIdFormaPago().getId() != null && pojoTarget.getIdFormaPago().getId() > 0L) {
				...
				pojoResult.setFormaPago(pojoTarget.getIdFormaPago().getFormaPago());
			}
			...
			if (pojoTarget.getIdCuentaDeposito() != null && pojoTarget.getIdCuentaDeposito().getId() > 0) {
				...
				pojoResult.setCuentaBanco(pojoTarget.getIdCuentaDeposito().getInstitucionBancaria().getNombreCorto());
				pojoResult.setCuentaNumero(pojoTarget.getIdCuentaDeposito().getNumeroDeCuenta());
			}
	...
	public FacturaPagosExt FacturaPagosToFacturaPagosExt(FacturaPagos pojoTarget) throws Exception {
		...
		try {
			...
			if (this.mostrarSystemOut) log.info("---- -- ----> Comprobando BancoDeposito");
			if (pojoTarget.getIdTimbre() > 0) {
				FacturaPagosTimbre pojoTimbre = this.ifzTimbresPagos.findById(pojoTarget.getIdTimbre());
				pojoResult.setIdTimbre(pojoTimbre);
			}
		} catch (Exception ex) {
