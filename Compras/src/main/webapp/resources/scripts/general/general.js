...
function cambiarResponsabilidad(success, pnlOperacion, pnlMensaje, urlRedirect) {
	if (success == false) {
...
function cambiarEmpresa(success, pnlOperacion, pnlMensaje, urlRedirect) {
	if (success == false) {
		RichFaces.ui.PopupPanel.hidePopupPanel(pnlMensaje);
		return;
	}
...
function cambiarModulo(event) {
	var url = document.referrer; // "https://disesa.giro26.net:9443/DISESA/index.faces;jsessionid=1nlu9mG7YdHT4sRRJKMy-Urr.undefined";
	...
	if (prevUrl == "https://disesa.giro26.net:" + puerto + "/DISESA/index.faces") {
