...
public class ComprobacionGastosAction {
	...
	public String eliminar(){
		try {
			...
			this.ifzComprobadoGto.delete(this.pojoComprobadoGto.getId());
	...
	public String guardar() {
		...
		try{
			...
			this.pojoComprobadoGto.setFacturado((this.facturado ? 1 : 0));
	...
	public String comprobar(){
		try{
			...
			this.listComprobadoGto = this.ifzComprobadoGto.findExtAll(this.pojoGtosComp.getId()); //.findLikePojoCompletoExt(this.pojoGtosComp.getId(), 0);
			actualizaTotalFaturas();
		}catch(Exception e){		
			this.tipoMensaje =5;
	...
	public String revisarComprobacionGasto(){
		...
		try{
			...
			//comprobado	 		
			pojoGtosComp = ifzGtosComp.actualizar(this.pojoGtosComp, "C", Calendar.getInstance().getTime());
	...
	public void analizarArchivo() throws Exception {
		...
		try {
			...
			respuesta = this.ifzComprobadoGto.cargaFacturaXML(this.fileSrc, TiposGastoCXP.ComprobacionGastos); //.analizaFactura(this.fileSrc);
			if (respuesta.getErrores().getCodigoError() != 0L) {
				this.facturaId = 0L;
	...
	public void setPojoComprobadoGto(PagosGastosDetExt pojoComprobadoGto) {
		try {
			...
			this.listDesgloseImpuestos 	= this.ifzDesgloImpto.findExtAll(this.pojoComprobadoGto.getId()); // findLikePojoCompletoExt(this.pojoComprobadoGto, 50);
