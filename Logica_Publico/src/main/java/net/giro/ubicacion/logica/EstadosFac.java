...
public class EstadosFac implements EstadosRem {
	...
	public long salvar(Estado estado) throws ExcepConstraint {
		if(estado.getId() == 0){
			return ifzEstado.save(estado, null);
		}else{
	...
	public List<Estado> saveOrUpdateList(List<Estado> entities) throws Exception {
		try {
			return this.ifzEstado.saveOrUpdateList(entities, null);
		} catch (Exception e) {
			log.error("error en Logica_Publico.FACADE.saveOrUpdateList(List<Estado> entities)", e);
			throw e;
		}
	}
