...
public abstract class DAOImpl<T extends Serializable> implements DAO<T> {
    ...
    public long save(T entity, Long idEmpresa) throws ExcepConstraint {
		...
		try {
			...
			log.error("RECHUM :: Error REFLECTION ", e);
		...
		try {
			...
			log.error("RECHUM :: Error ASIGNAR SECUENCIA :: " + tabla + " :: " + secuen, e);
		...
		try {
			...
			log.error("RECHUM :: Error GRABAR ENTIDAD :: " + tabla + " :: " + secuen, e);
		...
		for (T entity : entities) {
			...
			if (idStored <= 0) {
				try {
					...
					log.error("RECHUM :: Error ASIGNAR SECUENCIA :: " + tabla + " :: " + secuen + " [" + index + "]", e);
					throw new ExcepConstraint("ASIGNAR SECUENCIA", entity);			
				}
				...
				try {
					...
					log.error("RECHUM :: Error GRABAR ENTIDAD :: " + tabla + " :: " + secuen + " [" + index + "]", e);
					throw new ExcepConstraint("GRABAR ENTIDAD", entity);			
				}
			} else {
				// Actualizo
				try {
					...
					log.error("RECHUM :: Error ACTUALIZAR ENTIDAD :: " + tabla + " :: " + idStored + " [" + index + "]", e);
	...
	@Override
    public List<T> deleteAll(List<T> entities) throws Exception {
		@SuppressWarnings("rawtypes")
		Class clase = null;
		Method metodo = null;
		Long id = 0L;
		...
		for (T entity : entities) {
			try {
				clase = entity.getClass();
				metodo = clase.getMethod("getId");
				id = (Long) metodo.invoke(entity);
				if (id == null || id <= 0L)
					continue;
				this.delete(id);
				deleted.add(entity);
			} catch (Exception e) {
				log.error("No se puedo eliminar el entity " + clase + ": " + id, e);
			}
		}
