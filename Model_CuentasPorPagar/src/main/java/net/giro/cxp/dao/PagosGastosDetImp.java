...
public class PagosGastosDetImp extends DAOImpl<PagosGastosDet> implements PagosGastosDetDAO {
	...
	public List<PagosGastosDet> findAll(long idPagosGastos) {
		try {
			final String queryString = "select model from PagosGastosDet model where model.idPagosGastos.id = :idPagosGastos ";
			Query query = entityManager.createQuery(queryString);
			query.setParameter("idPagosGastos", idPagosGastos);
	...
	public List<PagosGastosDet> findByProperty(String propertyName, Object value, String orderBy) {
		String queryString = "select model from PagosGastosDet model where model." + propertyName + " = :propertyValue ";
		...
		try {	
			...
			if ("".equals(value.toString())) {
				...
				if  ("Proveedor".equals(propertyName)){
					v_where = "where ((lower(pag.beneficiario) like '%"+ value.toString().toLowerCase()+ "%') ";
	...
	public Double saldoRepCajaChica(Object value){
		try {
			Double res=0D;
			String queryString = "select sum(det.subtotal+det.totalImpuestos) from PagosGastosDet det inner join det.idPagosGastos pag where det.idPagosGastos=:propertyValue";
			Query query = entityManager.createQuery(queryString);
			query.setParameter("propertyValue", value);
	...
	public List<PagosGastosDet> findByFactura(long idFactura, boolean incluyeCanceladas) throws Exception {
		String queryString = "select model from PagosGastosDet model where model.idCfdi = :idFactura ";
		
		try {
			if (! incluyeCanceladas)
				queryString += "and model.idPagosGastos.estatus <> 'X' ";
			queryString += "order by model.idPagosGastos.fecha desc, model.fecha desc ";
			Query query = entityManager.createQuery(queryString);
