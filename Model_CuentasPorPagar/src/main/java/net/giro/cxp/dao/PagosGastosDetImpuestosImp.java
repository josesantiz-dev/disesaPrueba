...
public class PagosGastosDetImpuestosImp extends DAOImpl<PagosGastosDetImpuestos> implements PagosGastosDetImpuestosDAO  {
	...
	public List<PagosGastosDetImpuestos> findAll(long idPagosGastosDet) {		
		try {
			final String queryString = "select model from PagosGastosDetImpuestos model where model.idPagosGastosDet = :idPagosGastosDet ";
			Query query = entityManager.createQuery(queryString);
			query.setParameter("idPagosGastosDet", idPagosGastosDet);
			return query.getResultList();
		} catch (Exception re) {
			throw re;
		}
