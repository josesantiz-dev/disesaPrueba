...
public class CajaChicaFac implements CajaChicaRem{
	...
	PagosGastosRem ifzPagosGastos;
	PersonaRem ifzPersonas;
	SucursalesRem ifzSucursales;	
	ChequesRem ifzCheques;
	CtasBancoRem ifzCtasBancos;
	...
	public CajaChicaFac(){
		try {
    		...
    		this.ifzCtasBancos 	= (CtasBancoRem)	ctx.lookup("ejb:/Logica_TYG//CtasBancoFac!net.giro.tyg.logica.CtasBancoRem");
	...
	public Respuesta salvar(PagosGastosExt entityExt, boolean band, Long empresa) throws Exception{
		...
		try{
			...
			if(!this.ifzCtasBancos.haySaldoSuficiente(entityExt.getMonto(), entityExt.getIdCuentaOrigen().getId())){
				...
				reg = this.ifzPagosGastos.salvar(entityExt, band, empresa);
			}
	...
	public Respuesta cancelacion(PagosGastosExt entityExt, Date fech_modificacion, Short usuario, Long empresa) throws Exception{
		try{
			return this.ifzPagosGastos.cancelacion(entityExt, fech_modificacion, usuario, empresa);
		} catch(RuntimeException re) {
	...
	public Respuesta buscarCuentasBanco(String tipoBusqueda, String valorBusqueda){
		...
		try{
			...
			List<CtasBanco> listCuentasBanco = this.ifzCtasBancos.findLikeColumnName(tipoBusqueda, valorBusqueda);
			List<CtasBancoExt> listCuentasBancoExt = new ArrayList<CtasBancoExt>();
			
			for(CtasBanco pojoCtasBanco : listCuentasBanco){
	...
	public Respuesta buscarSucursales(String tipoBusqueda, String valorBusqueda){
		...
		try{ 
			...
			respuesta.getErrores().setCodigoError(0L);
			respuesta.getBody().addValor("listSucursales", listSucursalesExt);
		} catch (Exception e) {
