...
public class BancoAction implements Serializable {
	...
	public BancoAction() {
		try {
			...
			pojoBanco = new CatBancos();
			listBancos = new ArrayList<CatBancos>();
			this.numPagina = 1;
	...
	public long guardar() {
		try {
			...
			if(respuesta.getErrores().getCodigoError() == 0){
				pojoBanco = (CatBancos)respuesta.getBody().getValor("pojoBanco");
				if(id <= 0)
	...
	public void buscar() {
		try {
			...
			if(respuesta.getErrores().getCodigoError() == 0){
				this.listBancos = (List<CatBancos>)respuesta.getBody().getValor("listBancos");
	...
	public List<CatBancos> getListBancos() {
		return listBancos;
	}

	public void setListBancos(List<CatBancos> listBancos) {
