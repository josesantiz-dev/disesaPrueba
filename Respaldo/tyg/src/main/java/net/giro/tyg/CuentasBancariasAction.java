...
public class CuentasBancariasAction{
	...
	private CtasBanco pojoCuentasBancarias;
	private CatBancos pojoInstitucionesBancarias;
...
//contructor de la clase action en este caso
	public CuentasBancariasAction() throws Exception {
		...
		listValores = new ArrayList<CtasBanco>();
		this.numPagina=1;
		//numDoctoDigit = 1;
		valTipoBusqueda=""; 
		campoBusqueda="";
		...
		// parte del codigo que nos dira a donde esta haciendo referencia la
				// interfaz
		...
		pojoCuentasBancarias = new CtasBanco();
		pojoInstitucionesBancarias = new CatBancos();
		pojoSucursalesBancarias = new SucursalBancaria();
		pojoEmpresas = new Empresa();

		listCuentasBancarias = new ArrayList<CtasBanco>();
		listCatBancos = new ArrayList<CatBancos>();
	...
	public void guardar() {
		try {
			...
			if(respuesta.getErrores().getCodigoError() == 0){
				pojoCuentasBancarias = (CtasBanco)respuesta.getBody().getValor("pojoCuentaBanco");
				if(id <= 0){
	...
	public void buscar(){
		try {			
			...
			if(respuesta.getErrores().getCodigoError() == 0){
				this.listCuentasBancarias = (List<CtasBanco>)respuesta.getBody().getValor("listCuentaBancos");
	...
	public void cargarSelectBancos(){
		try{
			...
			if(respuesta.getErrores().getCodigoError() == 0){
				listCatBancos = (List<CatBancos>)respuesta.getBody().getValor("listBancos");
				listInstitucionesBancarias.clear();
				for (CatBancos i : listCatBancos) 
	...
	public CtasBanco getPojoCuentasBancarias() {
		return pojoCuentasBancarias;
	}
	public void setPojoCuentasBancarias(CtasBanco pojoCuentasBancarias) {
		this.pojoCuentasBancarias = pojoCuentasBancarias;
	}
	public CatBancos getPojoInstitucionesBancarias() {
		return pojoInstitucionesBancarias;
	}
	public void setPojoInstitucionesBancarias(CatBancos pojoInstitucionesBancarias) {
	...
	public List<CtasBanco> getListValores() {
		return listValores;
	}
	public void setListValores(List<CtasBanco> listValores) {
	...
	private CatBancos getBancosById(Long id, List<CatBancos> lista){
		for(CatBancos cv :lista){
			if(cv.getId() == id.longValue())
				return cv;
		}
		return null;
	...
	public List<CatBancos> getListCatBancos() {
		return listCatBancos;
	}

	public void setListCatBancos(List<CatBancos> listCatBancos) {
		this.listCatBancos = listCatBancos;
	}

	public List<CtasBanco> getListCuentasBancarias() {
		return listCuentasBancarias;
	}

	public void setListCuentasBancarias(List<CtasBanco> listCuentasBancarias) {
