...
public class MenusAction {
	...
	public void buscar(){
		try {
			...
			if(respuesta.getErrores().getCodigoError() == 0L)
				listMenus = (List<Menu>)respuesta.getBody().getValor("listMenus");
			else
	...
	public void buscarMenus(){
		try {
			this.resOperacion="";
			Respuesta respuesta = this.ifzAdministracion.buscarMenu("menu", this.valTipoBusqueda);
			if(respuesta.getErrores().getCodigoError() == 0L)
				listMenusSelec = (List<Menu>)respuesta.getBody().getValor("listMenus");
			else
				this.resOperacion = respuesta.getErrores().getDescError();
		} catch (Exception e) {
			resOperacion = problemInesp;
	...
	public void buscarFuncion(){
		try {
			this.resOperacion = "";
			Respuesta respuesta = this.ifzAdministracion.findLikeNombreAplicacion(busquedaFuncion, pojoAplicacion, 100);
			if(respuesta.getErrores().getCodigoError() == 0L)
				listFunciones = (List<Funcion>)respuesta.getBody().getValor("listFunciones");
			else
				this.resOperacion = respuesta.getErrores().getDescError();
		} catch (Exception e) {
			resOperacion = problemInesp;
	...
	private void cargarSelectAplicaciones(){
		...
		if(respuesta.getErrores().getCodigoError() == 0L){
			...
			listTmpAplicacion.clear();
			for (Aplicacion i : listAplicacion) {
				listTmpAplicacion.add(new SelectItem(i.getId(), i.getAplicacion()));
			}
		} else
			this.resOperacion = respuesta.getErrores().getDescError();
	}
	...
	public Long getSuggAplicacion() {
		return pojoMenu.getAplicaciones() != null ? pojoMenu.getAplicaciones().getId() : -1L;
	}
