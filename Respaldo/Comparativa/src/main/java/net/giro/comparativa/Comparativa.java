    ...
    public int generarReporte() throws Exception {
    	...
    	try {
            ...
            while (rsProductos.next()) {
        		...
        		// Precio Unitario
        		celda = row.createCell(noColumna, HSSFCell.CELL_TYPE_NUMERIC);
        		celda.setCellValue(rsProductos.getDouble("precio_unitario"));
        		celda.setCellStyle(sMoneda);
        		noColumna++;
        		
        		// Importe
        		celda = row.createCell(noColumna, HSSFCell.CELL_TYPE_NUMERIC);
        		celda.setCellValue(rsProductos.getDouble("importe"));
        		celda.setCellStyle(sMoneda);
        		noColumna++;
            			...
            			if (rsProvProductos.first()) {
	    					...
	    					do {
	    						if (rsProvProductos.getString("id_proveedor").equals(rsProveedores.getString("id_proveedor")) && rsProvProductos.getString("id_producto").equals(rsProductos.getString("id_producto"))) {
	    							...
	    							importe = rsProvProductos.getDouble("prov_importe");
	    	                		break;
	    						}
    	...
    	try {
            if (this.baseComparativa) {
                this.strQuery = "SELECT DISTINCT pro.id AS id_producto "
                        ...
                        + "    , det.cantidad "
                        + "    , COALESCE(CASE cot.id_requisicion WHEN 0 THEN fn_getPrecioUnitarioFromInsumo(ins.id, pro.id, pro.precio_compra) ELSE det.precio_unitario END, 0.0) AS precio_unitario "
                        + "    , (COALESCE(CASE cot.id_requisicion WHEN 0 THEN fn_getPrecioUnitarioFromInsumo(ins.id, pro.id, pro.precio_compra) ELSE det.precio_unitario END, 0.0) * det.cantidad) AS importe "
    ...
    private ResultSet getProveedoresProductos() throws Exception {
		...
		try {
			this.strQuery = "SELECT DISTINCT (cot.id_proveedor, det.id_producto) AS id_row "
					...
					+ "    LEFT  JOIN e769c352b7 pmo ON pmo.aa = cot.id_proveedor AND cot.tipo_persona_proveedor = 'N' "
					+ "$WHERE "
					+ "GROUP BY cot.id_proveedor, det.id_producto, CASE cot.tipo_persona_proveedor WHEN 'N' THEN pmo.af ELSE pfi.ag END, pro.descripcion "
