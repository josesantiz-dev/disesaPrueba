...
public class ConvertExt {
	...
	public ObraExt ObraToObraExt(Obra pojoTarget) throws Exception {
		...
		try {
			...
			if (this.mostrarSystemOut) log.info("---- -- ----> Comprobando ObraPrincipal");
			if (pojoTarget.getIdObraPrincipal() != null && pojoTarget.getIdObraPrincipal() > 0 && pojoTarget.getTipoObra() != 2) {
	...
	public ConceptoPresupuestoExt ConceptoPresupuestoToConceptoPresupuestoExt(ConceptoPresupuesto pojoTarget){
		ConceptoPresupuestoExt pojoResult = new ConceptoPresupuestoExt();
		
		return pojoResult;
	}
	
	public ConceptoPresupuesto ConceptoPresupuestoExtToConceptoPresupuesto(ConceptoPresupuestoExt pojoTarget){
		ConceptoPresupuesto pojoResult = new ConceptoPresupuesto();
