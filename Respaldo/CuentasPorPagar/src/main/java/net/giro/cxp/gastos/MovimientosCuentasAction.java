...
public class MovimientosCuentasAction implements Serializable {
	...
	private List<CtasBanco> listCtas;
	...
	public MovimientosCuentasAction() throws NamingException, Exception {
		...
		usuarioId = lm.getUsuarioResponsabilidad().getUsuario().getId(); //.getLoginBean().getUsuario().getUsuarioId();
		...
		listCtas = new ArrayList<CtasBanco>();
	...
	public String cancelar() {
		try{
			...
			resp = ifzMovCtas.cancelacion(this.pojoMovCtas, Calendar.getInstance().getTime(), Short.valueOf(String.valueOf(this.usuarioId)), 0L);
	...
	public String guardar() {
		try {
			...
			if (registroNuevo) {
				...
				res = this.ifzMovCtas.salvar(this.pojoMovCtas, false, null);
