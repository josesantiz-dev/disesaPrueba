...
public class ClientesFac implements ClientesRem {
	...
	public ClientesFac(){
		try {
			...
			ifzPersona 			= (PersonaDAO)					ctx.lookup("ejb:/Model_Clientes//PersonaImp!net.giro.clientes.dao.PersonaDAO");		
			ifzNombresAlternos 	= (PersonaNombresAlternoDAO)	ctx.lookup("ejb:/Model_Clientes//PersonaNombresAlternoImp!net.giro.clientes.dao.PersonaNombresAlternoDAO");		
			ifzEstudio 			= (PersonaEstudioDAO)			ctx.lookup("ejb:/Model_Clientes//PersonaEstudioImp!net.giro.clientes.dao.PersonaEstudioDAO");
			ifzDomicilios 		= (DomicilioDAO)				ctx.lookup("ejb:/Model_Clientes//DomicilioImp!net.giro.clientes.dao.DomicilioDAO");
			ifzDirectorio 		= (DirectorioTelefonicoDAO)		ctx.lookup("ejb:/Model_Clientes//DirectorioTelefonicoImp!net.giro.clientes.dao.DirectorioTelefonicoDAO");
			ifzIDentificacion 	= (IdentificacionDAO)			ctx.lookup("ejb:/Model_Clientes//IdentificacionImp!net.giro.clientes.dao.IdentificacionDAO");
			ifzPersonaNegocio 	= (PersonaNegocioDAO)			ctx.lookup("ejb:/Model_Clientes//PersonaNegocioImp!net.giro.clientes.dao.PersonaNegocioDAO");
			ifzNegocio 			= (NegocioDAO)					ctx.lookup("ejb:/Model_Clientes//NegocioImp!net.giro.clientes.dao.NegocioDAO");
			ifzContactoPersona 	= (ContactoPersonaDAO)			ctx.lookup("ejb:/Model_Clientes//ContactoPersonaImp!net.giro.clientes.dao.ContactoPersonaDAO");
			ifzContactoNegocio 	= (ContactoNegocioDAO)			ctx.lookup("ejb:/Model_Clientes//ContactoNegocioImp!net.giro.clientes.dao.ContactoNegocioDAO");
			...
			ifzMunicipio 		= (MunicipioDAO)				ctx.lookup("ejb:/Model_Publico//MunicipioImp!net.giro.plataforma.ubicacion.dao.MunicipioDAO");
			ifzBancos 			= (CatBancosDAO)				ctx.lookup("ejb:/Model_TYG//CatBancosImp!net.giro.tyg.dao.CatBancosDAO");
	...
	public long salvar(Domicilio domicilio) throws ExcepConstraint{
		if(domicilio.getId() > 0){
			...
			return ifzDomicilios.save(domicilio);
		}
			...
			if(domicilioExt.getColonia() != null){
				if(domicilioExt.getColonia().getId() > 0){
					...
					if(domicilio.getId() > 0){
						...
						domicilio.setId(ifzDomicilios.save(domicilio));
					}
	...
	public long salvar(Persona persona) throws ExcepConstraint {
		if(persona.getId() > 0){
			...
			return this.ifzPersona.save(persona);
		}
	...
	public Respuesta salvar(PersonaExt personaExt) throws ExcepConstraint {
		...
		try{
			...
			if(personaExt.getTipoPersona() == 2 && ! validaRFCMoral(personaExt.getRfc())) {
				...
				respuesta.getErrores().setDescError("Verifique, El RFC no es valido");
				respuesta.setBody(null);
				return respuesta;
			} 
			...
			if (personaExt.getRfc() != null && ! "".equals(personaExt.getRfc().trim())) {
				...
				pers = this.ifzPersona.findByColumnName("rfc", personaExt.getRfc());
				if (pers.size() > 0) {
					for (Persona p : pers) {
						if (p.getId() == personaExt.getId()) 
							continue;
						
						respuesta.getErrores().setCodigoError(Errores.ERROR_RFC_EXISTE);
						respuesta.getErrores().setDescError("Verifique, persona existente con el id: " + String.valueOf(p.getId()));
						respuesta.setBody(null);
						return respuesta;
					}
				}
			...
			if(persona.getId() > 0)
				...
				personaExt.setId(this.ifzPersona.save(persona));
	...
	public long salvar(DirectorioTelefonico directorio) throws ExcepConstraint {
		if(directorio.getId() > 0)
			...
			return this.ifzDirectorio.save(directorio);
	...
	public Respuesta salvar(DirectorioTelefonicoExt directorioExt) throws ExcepConstraint {
		...
		try{
			...
			if(directorio.getId() > 0){
				...
				directorio.setId(this.ifzDirectorio.save(directorio));
				directorioExt = convertDirectorioToDirectorioExt(directorio);
			}
	...
	public Respuesta salvar(PersonaNombresAlternoExt nombreAlternoExt) throws ExcepConstraint{
		...
		try{
			...
			if(nombreAlternoExt.getId() > 0){
				...
				nombreAlternoExt.setId(this.ifzNombresAlternos.save(nombreAlterno));
			}
	...
	public long salvar(PersonaEstudio estudio) throws ExcepConstraint {
		if(estudio.getId() > 0){
			...
			return this.ifzEstudio.save(estudio);
		}
		return estudio.getId();
	}
	
	@Override
	public long salvar(PersonaEstudioExt personaEstudioExt) throws ExcepConstraint {
		...
		if(personaEstudio.getId() > 0){
			...
			return this.ifzEstudio.save(personaEstudio);
		}
	...
	public long salvar(Identificacion identificacion) throws ExcepConstraint {
		if(identificacion.getId() > 0){
			...
			return this.ifzIDentificacion.save(identificacion) ;
		}
	...
	public Respuesta salvar(IdentificacionExt identificacionExt){
		...
		try {
			...
			if(identificacion.getId() == 0){
				...
				identificacion.setId(this.ifzIDentificacion.save(identificacion));
			} else
				this.ifzIDentificacion.update(identificacion);
	...
	public List<Persona> buscarPersona(String tipoBusqueda, String valorBusqueda) throws ExcepConstraint{
		return ifzPersona.findLikeColumnName(tipoBusqueda, valorBusqueda);
	}
	...
	public List<PersonaExt> buscarPersonaExt(String tipoBusqueda, String valorBusqueda,Long tipoPersonaBusqueda) throws ExcepConstraint{
		List<Persona> listPersonas = ifzPersona.findLikePersonaPropiedad(tipoBusqueda, valorBusqueda,tipoPersonaBusqueda);
	...
	@Override
	public Respuesta salvar(PersonaNegocioExt personaNegocioExt) throws ExcepConstraint {
		Respuesta respuesta = new Respuesta();
		try{
			PersonaNegocio personaNegocio = new PersonaNegocio();
			...
			if(personaNegocio.getId() > 0){
				...
				personaNegocioExt.setId(ifzPersonaNegocio.save(personaNegocio));
			}
	...
	public List<DomicilioExt> buscarDomicilioExt(Negocio negocio) throws ExcepConstraint{
		List<Domicilio> listDomicilios = this.ifzDomicilios.findByColumnName("negocio", negocio);
		List<DomicilioExt> listDomiciliosExt = new ArrayList<DomicilioExt>(); 
		for(Domicilio dom : listDomicilios){
			DomicilioExt domExt =  new DomicilioExt();
			domExt = convertDomicilioToDomicilioExt(dom);
			...
			if(dom.getPersona() != null){
				domExt.setPersona(convertPersonaToPersonaExt(dom.getPersona()));
			}
			Colonia col = new Colonia();
			col = ifzColonia.findById(dom.getColonia());
			domExt.setColonia(col);
			domExt.setNombreColonia(col.getNombre());			
			...
			if(dom.getNegocio() != null){
				domExt.setNegocio(dom.getNegocio());
			}*/
	...
	@Override
	public List<Negocio> buscarNegocios(String tipoBusquedaNegocio,String valorBusquedaNegocio, PersonaExt personaExt)throws ExcepConstraint {
	...
	public PersonaExt asignarPersona(NegocioExt negocioExt) throws ExcepConstraint {
	...
	@Override
	public Respuesta salvar(ContactoPersonaExt contactoPersonaExt) throws ExcepConstraint {
		Respuesta respuesta = new Respuesta();
		try{
			ContactoPersona contactoPersona = new ContactoPersona();
			...
			if(contactoPersonaExt.getId() > 0){
				...
				contactoPersonaExt.setId(ifzContactoPersona.save(contactoPersona));
			}
	...
	public Respuesta listarBancos(){
		Respuesta respuesta = new Respuesta();
		try{
			List<CatBancos> listBancos = ifzBancos.findAll();
			
			List<CatBancosExt> listBancosExt = new ArrayList<CatBancosExt>();
			for(CatBancos banco : listBancos){
