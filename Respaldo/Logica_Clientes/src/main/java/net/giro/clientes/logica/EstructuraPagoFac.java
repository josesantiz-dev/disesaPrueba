...
public class EstructuraPagoFac implements EstructuraPagoRem {
	...
	EstructuraImportacionCuentaDAO ifzEstructuraImportacion;
	CtasBancariasDAO ifzCtasBancos;
	...
	public EstructuraPagoFac() {
		try {
			...
			ifzCtasBancos  =  (CtasBancariasDAO)ctx.lookup("ejb:/Model_TYG//CtasBancariasImp!net.giro.tyg.dao.CtasBancariasDAO");
		} catch (Exception e) {
	...
	public Respuesta guardarEstructuraImportacion(EstructuraImportacionCuentaExt pojoEstructuraImportacionExt) {
		...
		try {
			...
			if (pojoEstructuraImportacion.getId() == 0L) {
				...
				pojoEstructuraImportacion.setId(ifzEstructuraImportacion.save(pojoEstructuraImportacion));
			} else
	...
	public Respuesta listarCtasBancos() {
		...
		try {
			...
			List<CtasBanco> listTmp;

			listTmp = ifzCtasBancos.findAll();

			for (CtasBanco o : listTmp) {
	...
	private CtasBancoExt convertCtasBancoToCtasBancoExt(long idCtasBanco) throws Exception{
		...
		try{
			CtasBanco pojoCtasBanco = ifzCtasBancos.findById(idCtasBanco);
