...
public class NegociosFac implements NegociosRem {
	...
	public NegociosFac(){
		try {
    		...
    		ifzPersonas = (PersonaDAO)ctx.lookup("ejb:/Model_Clientes//PersonaImp!net.giro.clientes.dao.PersonaDAO");
    		...
    		ifzCanalesNegocio = (CanalesNegocioDAO)ctx.lookup("ejb:/Model_Clientes//CanalesNegocioImp!net.giro.clientes.dao.CanalesNegocioDAO");
			...
			ifzEstadosNegocio = (EstadosNegocioDAO)ctx.lookup("ejb:/Model_Clientes//EstadosNegocioImp!net.giro.clientes.dao.EstadosNegocioDAO");
			
    		descEstatusNegocio = new HashMap<Long, String>();
    		descEstatusNegocio.put(ACTIVO, "Activo");
    		descEstatusNegocio.put(INACTIVO, "Inactivo");
    		
    		descEstatusContacto = new HashMap<Long, String>();
    		descEstatusContacto.put(ACTIVO, "Activo");
    		descEstatusContacto.put(INACTIVO, "Inactivo");
		} catch(Exception e) {
	...
	public List<Negocio> findAll() {
		try {
			return this.ifzNegocio.findAll();
		} catch (RuntimeException re) {
			throw re;
		}
	}
	...
	public List<Negocio> findAll(String nombre) throws Exception {
		try {
			return this.ifzNegocio.findAll(nombre);
		} catch (RuntimeException re) {
			throw re;
		}
	}
	...
	public Respuesta buscarGiro(Long idGiro){
		Respuesta respuesta = new Respuesta();
		try{
			ConValores pojoGiro = ifzConValores.findById(idGiro);
	...
	public List<ConValores> buscarGiros() throws ExcepConstraint{
		...
		List<ConGrupoValores> listaConGrupoValores = ifzConGrupoValores.findByColumnName("nombre","SYS_GIROS");
	...
	public Respuesta buscarGiros(String nombre) {
		...
		try{
			...
			List<ConValores> listGiros = ifzConValores.findByGrupoNombreLikeParams("SYS_GIROS", params);
	...
	public Respuesta buscarNegocios(String tipoBusquedaNegocio,String valorBusquedaNegocio){
		Respuesta respuesta = new Respuesta();
		try{
			List<Negocio> listNegocios;
			if(!tipoBusquedaNegocio.equals("id"))
				listNegocios = ifzNegocio.findLikeColumnName(tipoBusquedaNegocio, valorBusquedaNegocio);
			else
				listNegocios = ifzNegocio.findLikeId(valorBusquedaNegocio);
	...
	public Respuesta buscarPersonas(String tipoBusqueda, String valorBusqueda){
		...
		try {
			...
			if(!tipoBusqueda.equals("id"))
				listPersonas = ifzPersonas.findLikeColumnName(tipoBusqueda, valorBusqueda);
			else
				listPersonas = ifzPersonas.findLikeId(valorBusqueda);
	...
	public Respuesta salvarCanal(CanalesNegocioExt pojoCanalExt){
		...
		try{
			...
			if(pojoCanal.getId() == 0L){
				...
				pojoCanal.setId(ifzCanalesNegocio.save(pojoCanal));
			} else
	...
	public Respuesta salvarContacto(ContactoNegocioExt pojoContactoNegocioExt){
		...
		try{
			...
			if(pojoContactoNegocio.getId() > 0)
				...
				pojoContactoNegocio.setId(ifzContactoNegocio.save(pojoContactoNegocio));
			}
	...
	public Respuesta salvarEstadosNegocio(EstadosNegocioExt pojoEstadosNegocioExt){
		...
		try{
			...
			if(pojoEstadosNegocio.getId() > 0){
				...
				pojoEstadosNegocio.setId(ifzEstadosNegocio.save(pojoEstadosNegocio));
			}
	...
	public Respuesta salvarNegocio(Negocio negocio) {
		Respuesta respuesta = new Respuesta();
		try{
			if (negocio.getEstatus() != null && negocio.getEstatus() == INACTIVO){
				respuesta.getErrores().addCodigo("CLIENTES", Errores.ERROR_NEGOCIO_INACTIVO);
				respuesta.setBody(null);
				return respuesta;
			}
			...
			negocio.setModificadoPor(infoSesion.getAcceso().getId());
			negocio.setFechaModificacion(Calendar.getInstance().getTime());

			// Comprobamos que RFC ingresado no exista
			List<Negocio> negs = new ArrayList<Negocio>();
			negs = this.ifzNegocio.findByColumnName("rfc", negocio.getRfc());
			if (negs.size() > 0) {
				for (Negocio n : negs) {
					if (n.getId() == negocio.getId()) {
						continue;
					} else {
						respuesta.getErrores().setCodigoError(Errores.ERROR_RFC_EXISTE);
						respuesta.getErrores().setDescError("Verifique, negocio existente con el id: " + String.valueOf(n.getId()));
						respuesta.setBody(null);
						return respuesta;
					}
				}
			...
			if(negocio.getId() > 0){
				...
				negocio.setId(this.ifzNegocio.save(negocio));
			}
	...
	public Respuesta salvarDirectorio(DirectorioTelefonicoExt pojoDirectorioExt){
		...
		try{
			...
			if(pojoDirectorio.getId() == 0L){
				...
				pojoDirectorio.setId(ifzDirectorio.save(pojoDirectorio));
			} else
	...
	public Respuesta salvarDomicilio(Negocio pojoNegocio, DomicilioExt pojoDomicilioExt){
		...
		try{
			...
			if(pojoDomicilio.getId() > 0L){
				...
				pojoDomicilio.setId(ifzDomicilios.save(pojoDomicilio));
			}
	...
	public Respuesta salvarPersonaNegocio(PersonaNegocioExt personaNegocioExt){
		...
		try{
			...
			if(personaNegocio.getId() > 0){
				...
				personaNegocio.setId(ifzPersonaNegocio.save(personaNegocio));
			}
	...
	public List<Negocio> findByProperty(String propertyName, Object value, int limite) {
		try {
			return this.ifzNegocio.findByProperty(propertyName, value, limite);
		} catch (RuntimeException re) {
			throw re;
		}
	}
	...
	public List<Negocio> findLikeProperty(String propertyName, String value, int limite) {
		try {
			return this.ifzNegocio.findLikeProperty(propertyName, value, limite);
		} catch (RuntimeException re) {
			throw re;
		}
	}
