...
public class MonedaImp extends DAOImpl<Moneda> implements MonedaDAO  {
	...
	public List<Moneda> findByProperty(String propertyName, final Object value) {
		try {
			final String queryString = "select model from Moneda model where model."
	...
	public List<Moneda> findAll() {
		try {
			final String queryString = "select model from Moneda model order by id";
			Query query = entityManager.createQuery(queryString);
			return query.getResultList();
		} catch (RuntimeException re) {
			throw re;
	...
	public List<Moneda> findByPropertyLike(String propertyName, final String value) {
		try {
			String queryString = "select model from Moneda model where lower(model."
	...
	public Moneda findByAbreviacion(String valor) {
		try {
			final String queryString = "select model from Moneda model where model.abreviacion='"+ valor + "' order by model.abreviacion";			
			Query query = entityManager.createQuery(queryString);
