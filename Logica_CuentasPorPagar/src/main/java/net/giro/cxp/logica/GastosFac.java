...
public class GastosFac implements GastosRem {
	...
	public GastosFac() {
		Hashtable<String, Object> envoronment = null;
		
		try {
			envoronment = new Hashtable<String, Object>();
			envoronment.put(Context.URL_PKG_PREFIXES, "org.jboss.ejb.client.naming");
			this.ctx = new InitialContext(envoronment);
	...
	public List<GastosExt> findAll() throws Exception {
		...
		try {
			...
			this.ifzConValores.setInfoSesion(this.infoSesion);
			lista = this.ifzConValores.findAll(this.pojoGrupo);
	...
	public List<GastosExt> findByProperty(String propertyName, Object value, int limite) throws Exception {
		...
		try {
			...
			this.ifzConValores.setInfoSesion(this.infoSesion);
			lista = this.ifzConValores.findByProperty(propertyName, value, this.pojoGrupo, 0);
			if (lista == null || lista.isEmpty())
				return listaResult;
	...
	public List<GastosExt> findLikeProperty(String propertyName, String value, int limite) throws Exception {
		...
		try {
			...
			this.ifzConValores.setInfoSesion(this.infoSesion);
			lista = this.ifzConValores.findLikeProperty(propertyName, value, this.pojoGrupo, 0);
			if (lista == null || lista.isEmpty())
				return listaResult;
