...
public class BackOrderOrdenes implements Serializable {
    ...
    public int generarReporte() throws Exception {
    	...
    	try {
            ...
            do {
        		...
        		noColumna++;
        		celda = row.createCell(noColumna, HSSFCell.CELL_TYPE_NUMERIC);
        		celda.setCellFormula("H" + (currentRow + 1) + "*" + "F" + (currentRow + 1));
        		celda.setCellStyle(this.sNumero);
    ...
    private ResultSet getOrdenCompra() throws Exception {
		try {
			...
			this.strQuery = "SELECT oc.id AS id_orden_compra"
						...
						+ ", oc.folio, DATE(oc.fecha) AS fecha"
						+ ", oc.anticipo"
						+ ", oc.tipo_cambio"
						+ ", oc.plazo, oc.tiempo_entrega, oc.lugar_entrega"
						+ ", o.id_obra, o.nombre AS obra"
						+ ", oc.id_proveedor, CASE cot.tipo_persona_proveedor WHEN 'N' THEN nprv.af ELSE prv.ag END AS proveedor"
						+ ", cto.ax AS correo"
						...
						+ ", cto.ag AS contacto, sta.nombre AS solicita, com.nombre AS comprador"
						+ ", pro.id AS id_producto, pro.clave AS codigo pro.descripcion"
						+ ", pro.unidad_medida AS id_unidad_medida, CASE COALESCE(med.aa,0) WHEN 0 THEN '' ELSE med.af END AS unidad_medida"
						+ ", det.cantidad, det.precio_unitario, det.importe"
						...
						+ ", usu.ag AS autorizo"
						+ ", oc.id_moneda, mo.abreviacion AS moneda_abr, mo.nombre || ' (' || mo.abreviacion || ')' AS moneda"
						+ ", 0.00 AS descuento"
						+ ", COALESCE((SELECT COALESCE(SUM(cantidad), 0) "
									...
									+ "WHERE y.id_orden_compra = oc.id and z.id_producto = det.id_producto and y.tipo = 0 and y.owner <= 0 and y.estatus = 0), 0) AS surtido "
						+ ", CURRENT_DATE AS fecha_emision"
					+ "FROM orden_compra oc INNER JOIN orden_compra_detalle det ON det.id_orden_compra = oc.id "
    ...
    private ResultSet getFacturas() throws Exception {
		try { 
			// Generamos consulta 
			this.strQuery  = "SELECT y.id AS id_movimiento, 'F-' || y.folio_factura as folio_factura, z.id_producto, z.cantidad AS cantidad_surtida ";
			this.strQuery += "FROM almacen_movimientos y INNER JOIN movimientos_detalle z ON z.id_almacen_movimiento = y.id ";
			this.strQuery += "WHERE y.id_orden_compra = $idOrdenCompra and y.tipo = 0 and y.owner <= 0 and y.estatus = 0";
			this.strQuery  = this.strQuery.replace("$idOrdenCompra", this.params.get("idOrdenCompra"));
