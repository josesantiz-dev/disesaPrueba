...
public class NegociosAction implements Serializable {
	private Logger log = Logger.getLogger(NegociosAction.class);
	...
	// validadcion
	private String rfcExpresionRegular;
	private String rfcInvalido;
	...
	public NegociosAction() throws Exception {
		...
		try {
			...
			this.rfcExpresionRegular = propPlataforma.getString("rfcFull");
			if (this.rfcExpresionRegular == null || "".equals(this.rfcExpresionRegular.trim()))
				this.rfcExpresionRegular = "(^[\\w&]{3,4}(\\d{2}((?:0[1-9]|1[0-2])(?:0[1-9]|[1,2]\\d|3[0,1])))([A-Z\\d]{2})([A\\d]))";
			...
			this.sinSocios = propPlataforma.getString("mensaje.validacion.sinSocios");
			this.inicioOperaciones = propPlataforma.getString("mensaje.validacion.inicioOperacion");
			this.participacionSocios =  propPlataforma.getString("mensaje.validacion.participacionSocios");
			this.participacionSocio =  propPlataforma.getString("mensaje.validacion.participacionSocio");
			this.rfcInvalido = propPlataforma.getString("mensaje.validacion.rfc");
			this.rfcFisica = propPlataforma.getString("mensaje.validacion.rfcF");
			this.rfcMoral = propPlataforma.getString("mensaje.validacion.rfcM");
	

			// Inicializaciones
			this.pojoNegocio = new Negocio();
			this.pojoGiro = new ConValores();
			...
			this.pojoPersona = new PersonaExt();
			this.pojoEstadoDom = new Estado();
			this.pojoCanalesNegocio = new CanalesNegocioExt();
			this.pojoContactoNegocio = new ContactoNegocioExt();
			this.pojoContactoNegocioEst = new ContactoNegocioExt();
			this.pojoEstadosNegocio = new EstadosNegocioExt();
		
			this.listGiros = new ArrayList<SelectItem>();				
			this.listSectores = new ArrayList<SelectItem>();
			this.listColonia = new ArrayList<SelectItem>();
			this.listEstados = new ArrayList<SelectItem>();
			this.listNegocios = new ArrayList<Negocio>();	
			this.listPersonasAsociadas = new ArrayList<PersonaNegocioExt>();
			this.listPersona = new ArrayList<PersonaExt>();
			this.listPersonaContacto = new ArrayList<PersonaExt>();
			this.listContactos = new ArrayList<ContactoNegocioExt>();
			this.listEstadosNegocio = new ArrayList<EstadosNegocioExt>();
			this.listGirosC = new ArrayList<ConValores>();
			this.listBancos = new ArrayList<CatBancosExt>();

			this.numPagina = 1;
			this.numPaginaDomicilio = 1;
			this.numPaginaDirectorio = 1;
			this.numPaginaBuscarApoderado = 1;
			this.numPaginaBuscarPersona = 1;
			this.numPaginaPersonaAsociada = 1;
			this.numPaginaLocalidad = 1;
			this.numPaginaMunicipio = 1;
			this.numPaginaColonia = 1;
			this.numPaginaCanales = 1;
			this.numPaginaPersonasContactos = 1;
			this.numPaginaEstadosNegocio = 1;
			this.numPaginaGiro = 1;

			this.descEstatusNegocio = this.ifzNegocios.getDescEstatusNegocio();
			this.descEstatusContacto = this.ifzNegocios.getDescEstatusContacto();
			...
			for (Estado e : this.listTmpEstado) {
				if (this.pojoEstadoNac == null)
					this.pojoEstadoNac = e;
			...
			log.error("Ocurrio un problema al intentar inicializar NegociosAction", e);
		}
	}
	...
	public void guardar() {
		...
		try {
			...
			if (this.pojoNegocio.getId() == null || this.pojoNegocio.getId() <= 0L) {
				this.pojoNegocio.setId(0L);
				nuevo = true;
			}
			...
			this.ifzNegocios.setInfoSesion(this.loginManager.getInfoSesion());
			respuesta = this.ifzNegocios.salvarNegocio(this.pojoNegocio);
			if (respuesta.getErrores().getCodigoError() != 0L) {
				this.resOperacion = respuesta.getErrores().getDescError();//Errores.descError.get(respuesta.getErrores().obtenerUltimoError());
				return;
			}
			
			this.pojoNegocio = (Negocio) respuesta.getBody().getValor("pojoNegocio");
			if (nuevo) 
				this.listNegocios.add(0, this.pojoNegocio);
			guardarListaAsociados();
		} catch (Exception e) {
			this.resOperacion = this.problemInesp;
			log.error("Error en el metodo guardar ... " + this.resOperacion, e);
		}
	}
	...
	private boolean validarNegocio() {
		Pattern pattern = null;
		Matcher matcher = null;

		pattern = Pattern.compile(this.rfcExpresionRegular);
		matcher = pattern.matcher(this.pojoNegocio.getRfc());
		if (! matcher.find()) {
			if ("M".equals(this.pojoNegocio.getTipoPersonalidad()))
				this.resOperacion = this.rfcInvalido + ". " + this.rfcMoral;
			else if ("M".equals(this.pojoNegocio.getTipoPersonalidad()))
				this.resOperacion = this.rfcInvalido + ". " + this.rfcFisica;
			else 
				this.resOperacion = this.rfcInvalido;
			return false;
		}
	...
	public void nuevoContacto() {
		this.pojoContactoNegocio = new ContactoNegocioExt();
		this.pojoContactoNegocio.setIdNegocio(this.pojoNegocio);
	}
	...
	public void cargarBancos() {
		try {
			...
			if (respuesta.getErrores().getCodigoError() == 0L) {
				this.listBancos = (List<CatBancosExt>) respuesta.getBody().getValor("listBancos");
			...
			if (! this.listPersonasAsociadas.isEmpty()) {
				for (PersonaNegocioExt personaNegocio : this.listPersonasAsociadas) {
					if (personaNegocio.getNegocio() == null) {
						personaNegocio.setNegocio(this.pojoNegocio);
						respuesta = this.ifzNegocios.salvarPersonaNegocio(personaNegocio);
						if (respuesta.getErrores().getCodigoError() == 0L)
    ...
    public List<SelectItem> getListColonia() {
		return listColonia;
	}
