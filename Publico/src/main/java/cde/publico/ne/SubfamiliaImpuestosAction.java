...
public class SubfamiliaImpuestosAction implements Serializable {
	...
	public void guardar() {
		try{
			...
			if (this.pojoSubfamiliaImpuestos.getValor() == null)
				this.pojoSubfamiliaImpuestos.setValor(BigDecimal.ZERO);
			this.pojoSubfamiliaImpuestos.setIdEmpresa(this.loginManager.getInfoSesion().getEmpresa().getId());
			...
			if (this.pojoSubfamiliaImpuestos.getId() == null || this.pojoSubfamiliaImpuestos.getId() <= 0L) {
				...
				this.pojoSubfamiliaImpuestos.setId(this.ifzSubfamiliaImpuestos.save(this.pojoSubfamiliaImpuestos));
	...
	public void eliminar() {
		try {
			control();
			if (this.pojoSubfamiliaImpuestosBorrar != null) {
				log.warn("Eliminando asignacion de Impuesto " + this.pojoSubfamiliaImpuestosBorrar.getDescImpuesto() + " (" + this.pojoSubfamiliaImpuestosBorrar.getIdImpuesto() + ") para la Familia/SubFamilia " + this.pojoSubfamiliaImpuestosBorrar.getDescSubfamilia() + " (" + this.pojoSubfamiliaImpuestosBorrar.getIdSubfamilia() + ")");
				if (this.pojoSubfamiliaImpuestosBorrar.getId() != null && this.pojoSubfamiliaImpuestosBorrar.getId() > 0L)
					this.ifzSubfamiliaImpuestos.delete(this.pojoSubfamiliaImpuestosBorrar.getId());
				this.listSubfamiliasImpuestos.remove(this.pojoSubfamiliaImpuestosBorrar);
			}
		} catch (Exception e) {
			log.error("Ocurrio un problema al intentar eliminar la asignacion de impuesto a la Familia/Subfamilia seleccionada", e);
			control("Ocurrio un problema al intentar eliminar la asignacion de impuesto a la Familia/Subfamilia seleccionada");
		}
	}
	...
	private boolean validamosSubfamiliaImpuesto() {
		...
		if (this.listSubfamiliasImpuestos == null)
			this.listSubfamiliasImpuestos = new ArrayList<SubfamiliaImpuestos>();
	...
	public void cargarSubfamilias() {
		...
		try {
			...
			for (ConValores var : listEspecialidades) {
				...
				this.listSubfamilias.addAll(listAux);
			...
			if (this.listSubfamilias != null && ! this.listSubfamilias.isEmpty()) {
				...
				for (ConValores var : this.listSubfamilias) {
					if (var.getDescripcion() == null)
						continue;
					this.listSubfamiliasItems.add(new SelectItem(var.getId(), var.getDescripcion() + " (" + var.getId() + ")"));
				}
			}
		} catch (Exception e) {
		...
		try {
			...
			if (this.listImpuestos != null && ! this.listImpuestos.isEmpty()) {
				...
				for (ConValores var : this.listImpuestos) {
					if (var.getDescripcion() == null)
						continue;
					this.listImpuestosItems.add(new SelectItem(var.getId(), var.getDescripcion() + " (" + var.getId() + ")"));
				}
			}
		} catch (Exception e) {
