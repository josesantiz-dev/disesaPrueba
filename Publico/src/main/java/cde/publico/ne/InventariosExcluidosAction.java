...
public class InventariosExcluidosAction implements Serializable {
	private static Logger log = Logger.getLogger(InventariosExcluidosAction.class);
	...
	public InventariosExcluidosAction() throws Exception{
			...
			if (this.listSubfamilias != null && ! this.listSubfamilias.isEmpty()) {
				...
				for (ConValores var : this.listSubfamilias) {
					if (var.getDescripcion() == null)
						continue;
					this.listSubfamiliasItems.add(new SelectItem(var.getId(), var.getId() + " - " + var.getDescripcion()));
				}
			}
		} catch (Exception e) {
		...
		try {
			...
			if (this.listImpuestos != null && ! this.listImpuestos.isEmpty()) {
				...
				for (ConValores var : this.listImpuestos) {
					if (var.getDescripcion() == null)
						continue;
					this.listImpuestosItems.add(new SelectItem(var.getId(), var.getId() + " - " + var.getDescripcion()));
				}
			}
		} catch (Exception e) {
