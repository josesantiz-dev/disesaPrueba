...
public class ObraAvancesAction implements Serializable {
	private static final long serialVersionUID = 1L;
	private static Logger log = Logger.getLogger(ObraAvancesAction.class);	
	...
	public void eliminar() {
		...
		try {
			...
			if (this.pojoObraAvanceBorrar != null) {
				if (! formatter.format(Calendar.getInstance().getTime()).equals(formatter.format(this.pojoObraAvanceBorrar.getFecha()))) {
					control(true, -1, "No se permite eliminar registros con fecha distinta a la registrada en el Avance");
					return;
				}
