...
public class ObrasAction implements Serializable {
	...
	// Estados
	private List<Estado> listEstadosItems;
	...
	// cargaTiposDomicilio
	private List<ConValores> listTiposDomicilio1;
	private List<SelectItem> listTiposDomicilioItems1;
	private List<ConValores> listTiposDomicilio2;
	private List<SelectItem> listTiposDomicilioItems2;
	// Listas auxiliares
	...
	private boolean incluyeCanceladas;
    ...
    public ObrasAction() {
    	...
    	try {
			...
			this.listTiposDomicilio1 	= new ArrayList<ConValores>();
			this.listTiposDomicilio2 	= new ArrayList<ConValores>();			
			...
			this.listTiposDomicilioItems1 	= new ArrayList<SelectItem>();
			this.listTiposDomicilioItems2 	= new ArrayList<SelectItem>();
			...
			log.error("Ocurrio un problema al incializar " + this.getClass().getCanonicalName(), e);
		} finally {
			establecerPermisos();
		}
    }
    ...
    public void buscar() {
    	try {
    		control();
    		this.campoBusqueda = (this.campoBusqueda != null && !"".equals(this.campoBusqueda.trim()) ? this.campoBusqueda : this.tiposBusqueda.get(0).getValue().toString());
			this.tipoObraBusqueda = (this.busquedaAdministrativas ? 4 : 0);
    		controlLog("Buscando Obra. " + this.campoBusqueda + ": " + this.valorBusqueda);
    		this.numPagina = 1;
    		this.ifzObras.setInfoSesion(this.loginManager.getInfoSesion());
    		if ("nombreObraPrincipal".equals(this.campoBusqueda))
    			this.listObrasGrid = this.ifzObras.findPrincipalLikeProperty("nombre", this.valorBusqueda, this.tipoObraBusqueda, this.incluyeCanceladas, "", 0);
    		else
    			this.listObrasGrid = this.ifzObras.findLikeProperty(this.campoBusqueda, this.valorBusqueda, this.tipoObraBusqueda, this.incluyeCanceladas, "", 0);
			if (this.listObrasGrid == null || this.listObrasGrid.isEmpty())
	    		control(2, "Busqueda sin resultados");
    	} catch (Exception e) {
    		control("Ocurrio un problema al intentar consultar las Obras", e);
    	} finally {
    		this.listObrasGrid = (this.listObrasGrid != null ? this.listObrasGrid : new ArrayList<Obra>());
			controlLog(this.listObrasGrid.size() + " Obras encontradas");
    	}
    }
    ...
    public void nuevo() {
    	...
    	try {
			...
			this.pojoObra.setFechaInicio(Calendar.getInstance().getTime());
			this.pojoObra.setFechaTerminacion(calendar.getTime());
    ...
    public void editar() {
    	try {
    		...
    		controlLog("Recuperando Obra para editar: " + this.pojoObraMain.getId());
			...
			if (this.listEstatusObras == null || this.listEstatusObras.isEmpty()) 
				if (! cargaEstatusObras())
					return;
			
			if (this.listEstatusContratoItems == null || this.listEstatusContratoItems.isEmpty()) 
				if (! cargaEstatusContrato())
					return;
    ...
    public void guardar() {
    	try {
			...
			if (this.pojoObra.getId() == null || this.pojoObra.getId() <= 0L) {
				...
				this.listObrasGrid.add(0, this.pojoObraMain);
			} else {
				...
				for (Obra var : this.listObrasGrid) {
					if (var.getId().longValue() == this.pojoObra.getId().longValue()) {
						this.listObrasGrid.set(this.listObrasGrid.indexOf(var), this.ifzObras.convertir(this.pojoObra));
						break;
					}
				}
			}
    ...
    public void eliminar() {
    	...
    	try {
    		control();
    		if (this.pojoObraMain == null) {
    			control(-1, "Ocurrio un problema al recuperar la Obra indicada");
    			return;
    		}
    		
    		this.ifzObras.setInfoSesion(this.loginManager.getInfoSesion());
    		resp = this.ifzObras.cancelar(this.pojoObraMain);
    		if (resp.getErrores().getCodigoError() != 0) {
    			if (resp.getErrores().getCodigoError() == 1L) {
    				control(-1, "Ocurrio un problema al intentar cancelar la Obra.");
    				return;
    			}
    			
				control(-1, "Cancelacion no permitida.\n" + resp.getErrores().getDescError());
    			return;
    		}
    		
    		this.pojoObraMain = (Obra) resp.getBody().getValor("obra");
    		if (this.pojoObraMain == null)
    			return;
    		for (Obra ob : this.listObrasGrid) {
    			if (this.pojoObraMain.getId().longValue() == ob.getId().longValue()) {
    				this.listObrasGrid.set(this.listObrasGrid.indexOf(ob), this.pojoObraMain);
    				break;
    			}
    		}
    		...
    		control(true, -1, "Ocurrio un problema al intentar Cancelar la Obra seleccionada", e);
    	}
    }
    ...
    private boolean validaciones() {
		...
		if (this.pojoObra.getMontoAnticipo().doubleValue() < 0) {
			// La Arq. Carolina me pidio por correo que no importa el tipo, se permite cero o mayor
    	...
    	if (this.pojoObra.getIdMoneda() == null || this.pojoObra.getIdMoneda() <= 0L) {
    		control(-1, "Debe seleccionar una Moneda");
			return false;
    	}
    	...
    	if (this.pojoObra.getFechaInicio().equals(this.pojoObra.getFechaTerminacion()) || this.pojoObra.getFechaTerminacion().before(this.pojoObra.getFechaInicio())) {
    		control(-1, "La Fecha de Terminacion debe ser Mayor a la de Inicio");
			return false;
    	}
		...
		if (this.pojoObra.getTipoCambio() <= 0)
			this.pojoObra.setTipoCambio(1);
		if (this.pojoObra.getTipoCambioAnticipo() <= 0)
			this.pojoObra.setTipoCambioAnticipo(1);
		if (this.pojoObra.getTipoCambioDeductiva() <= 0)
			this.pojoObra.setTipoCambioDeductiva(1);
    ...
    public void buscarMunicipios() {
    	try {
			...
			this.listMunicipios = this.ifzClientes.buscarMunicipio(this.pojoEstado, this.busquedaMunicipio);
			if (this.listMunicipios.isEmpty()) 
				control(2, "Busqueda sin resultados");
    	} catch (Exception e) {
    		control("Ocurrio un problema al intentar consultar los Municipios", e);
    	}
    }
    ...
    public void buscarLocalidades() {
    	try {
			...
			control("Ocurrio un problema al intentar consultar las Localidades", e);
    	}
    }
    ...
    public void buscarColonias() {
    	try {
			...
			control("Ocurrio un problema al intentar consultar las Colonias", e);
    	}
    }
			...
			if (this.pojoObra != null && this.pojoObra.getId() != null && this.pojoObra.getId() > 0L) {
				for (Obra var : this.listObrasGrid) {
					if (this.pojoObra.getId() == var.getId()) {
						...
						this.pojoObraMain = this.ifzObras.revisado(var, this.loginManager.getUsuarioResponsabilidad().getUsuario().getId(), Calendar.getInstance().getTime());
    ...
    public boolean guardarDomicilio() {
		...
		try {
			...
			this.pojoDomicilioExt.setColonia(this.pojoColonia);
    ...
    public boolean cargaEstatusObras() {
    	try {
			...
			Collections.sort(this.listEstatusObras, new Comparator<ConValores>() {
				@Override
				public int compare(ConValores o1, ConValores o2) {
					return o1.getAtributo1().compareTo(o2.getAtributo1());
				}});
    ...
    public boolean cargaEstados() {
    	try {
			control();
			this.listEstados = new ArrayList<SelectItem>();
			this.listEstadosItems = this.ifzClientes.buscarEstados();
			if (this.listEstadosItems == null || this.listEstadosItems.isEmpty()) {
				control(-1, "No se encontro ningun estado.");
				return false;
			}

			for (Estado e : this.listEstadosItems) 
				this.listEstados.add(new SelectItem(e.getId(), e.getNombre()));
		} catch (Exception e) {
    ...
    public void cargaObrasEmpleados() {
    	try {
    		control();
			this.listObraEmpleadosGrid = this.ifzObraEmpleados.findExtAll(this.pojoObra.getId());
    	} catch (Exception e) {
    ...
    public void evaluaTiposCambio() {
    	if (this.pojoObra == null || this.pojoObra.getIdMoneda() == null || this.pojoObra.getIdMoneda() <= 0L)
    		return;
    	if (getMostrarTipoCambio()) {
    		if (this.pojoObra.getMontoContratado().doubleValue() > 0 && this.pojoObra.getTipoCambio() <= 1)
    			this.pojoObra.setTipoCambio(this.loginManager.getTipoCambioActual());
    		if (this.pojoObra.getMontoAnticipo().doubleValue() > 0 && this.pojoObra.getTipoCambioAnticipo() <= 1)
    			this.pojoObra.setTipoCambioAnticipo(this.loginManager.getTipoCambioActual());
    		if (this.pojoObra.getMontoDeductiva().doubleValue() > 0 && this.pojoObra.getTipoCambioDeductiva() <= 1)
    			this.pojoObra.setTipoCambioDeductiva(this.loginManager.getTipoCambioActual());
    	} else {
			this.pojoObra.setTipoCambio(0.0);
    		if (this.pojoObra.getMontoContratado().doubleValue() > 0)
    			this.pojoObra.setTipoCambio(1.0);
			this.pojoObra.setTipoCambioAnticipo(0.0);
    		if (this.pojoObra.getMontoAnticipo().doubleValue() > 0)
    			this.pojoObra.setTipoCambioAnticipo(1.0);
			this.pojoObra.setTipoCambioDeductiva(0.0);
    		if (this.pojoObra.getMontoDeductiva().doubleValue() > 0)
    			this.pojoObra.setTipoCambioDeductiva(1.0);
    	}
    }

    public void evaluaEstatusObra() {
    	this.pojoObra.setFechaEstatus(null);
    	if (this.pojoObra.getEstatus() != null && this.pojoObra.getEstatus() > 0L) {
        	for (ConValores item : this.listEstatusObras) {
        		if (this.pojoObra.getEstatus().longValue() == item.getId()) {
        	    	this.pojoObra.setFechaEstatus("S".equals(item.getAtributo2()) ? Calendar.getInstance().getTime() : null);
        	    	break;
        		}
        	}
    	}
    ...
    public void evaluaEstatusContrato() {
    ...
    private void cargaTiposDomicilio() {
		...
		try {
			...
			cargaTipoDom(params, this.listTiposDomicilioItems1, this.listTiposDomicilio1);

			params = new HashMap<String, String>();
			params.put("atributo2", "2");
			cargaTipoDom(params, this.listTiposDomicilioItems2, this.listTiposDomicilio2);
		} catch (Exception e) {
	...
	private void cargaTipoDom(HashMap<String, String> params, List<SelectItem> items, List<ConValores> listDom) {
		try {
    		control();
			listDom.addAll(this.ifzClientes.buscarDomicilios(params));
    ...
    public void buscarObras() {
    	...
    	try {
			...
			if ("".equals(this.campoBusquedaObras))
				this.campoBusquedaObras = this.tiposBusquedaObras.get(0).getValue().toString();
			controlLog("Buscando Obras principales");
    		this.ifzObras.setInfoSesion(this.loginManager.getInfoSesion());
			this.listObrasPrincipales = this.ifzObras.findPrincipalLikeProperty(this.campoBusquedaObras, this.valorBusquedaObras, tipoObra, false, null, 0);
    		...
    		if (this.listObrasPrincipales == null)
    			this.listObrasPrincipales = new ArrayList<Obra>();
			controlLog(listObrasPrincipales.size() + " obras encontradas");
    	}
    }
    ...
    public void nuevaBusquedaClientes() {
		this.campoBusquedaClientes = this.tiposBusquedaClientes.get(0).getValue().toString();
    ...
    public void buscarClientes() {
    	try {
    		...
    		this.listClientes = (this.listClientes != null ? this.listClientes : new ArrayList<Persona>());
			controlLog(this.listClientes.size() + " clientes (" + (("P".equals(this.valorOpcionBusquedaClientes)) ? "Personas" : "Negocios") + ") encontrados");
    	}
    }

    public void seleccionarCliente() {
    	PersonaExt cliente = null;
    	
    	try {
			controlLog("Extiendo cliente seleccionado");
			cliente = this.ifzObras.extenderPersona(this.pojoCliente);
			if (cliente == null) {
				controlLog("No pude extender el cliente seleccionado - ERROR INTERNO");
				control(-1, "ERROR INTERNO - No se pudo recuperar el Cliente seleccionado");
				return;
			}

			controlLog("Asigno cliente a obra");
			this.pojoObra.setIdCliente(cliente);
    ...
    public void nuevaBusquedaResponsables() {
		this.campoBusquedaResponsables = this.tiposBusquedaResponsables.get(0).getValue().toString();
	...
	public void buscarResponsables() {
		try {
			...
			controlLog(this.listResponsablesGrid.size() + " responsables encontrados");
		} catch (Exception e) {
    ...
    public void seleccionarResponsable() {
    	...
    	try {
			controlLog("Extiendo responsable (Empleado) seleccionado");
			pojoExt = this.ifzEmpleados.convertir(this.pojoResponsable);
			if (pojoExt == null) {
				controlLog("No pude extender el responsable (Empleado) seleccionado - ERROR INTERNO");
				control(-1, "ERROR INTERNO - No se pudo recuperar el responsable (Empleado) seleccionado");
				return;
			}

			controlLog("Asigno responsable (Empleado) a obra");
    	...
    	try {
    		...
    		if (! presupuestos.isEmpty()) {	//debe tener uno para considerar que ya se tiene un presupuesto agregado a esta obra
        		...
        		if (this.listaPresupuestoGrid.size() == 16) {
            		...
            		if (this.isDebug) 
            			controlLog("Ya existe presupuesto para esta obra");
    ...
    private void cargarConceptosPresupuesto() {
    	...
    	if (this.isDebug) 
    		controlLog("Carga de Conceptos: " + this.listaConceptosPresupuesto.size());
    ...
    public void calcularMontosOLD() {
    	...
    	*/
    	...
    	try{ // this.listaPresupuestoGrid = this.ifzPresupuestoDetalle.findByPropertyExt("idPresupuesto", this.pojoPresupuesto.getId(), 0);
    		listaInsumos = this.ifzInsumos.findExtByActivos(this.pojoObra.getId());
    		if ( listaInsumos.isEmpty() ) return;
    		if (this.isDebug) 
    			controlLog("Calculando insumos");
    ...
    public void guardarPresupuesto() {
    	...
    	try {
    		...
    		this.pojoPresupuesto.setModificadoPor(this.loginManager.getUsuarioResponsabilidad().getUsuario().getId());
    		...
    		if (this.pojoPresupuesto.getId() == null) {			//Guardar; se compara con cero porque es un objeto de tipo primitivo
    			this.pojoPresupuesto.setCreadoPor(this.loginManager.getUsuarioResponsabilidad().getUsuario().getId());
		...
		try {
			...
			if (this.pojoObra != null && this.pojoObra.getId() != null && this.pojoObra.getId() > 0L) {
				...
				if (respuesta.getErrores().getCodigoError() == 0L) {
					...
					controlLog(respuesta.getErrores().getDescError());
					control(-1, respuesta.getErrores().getDescError());
				}
			}
		} catch (Exception e) {
    ...
    public void buscarEmpleados() {
    	try {
			...
			controlLog(this.listEmpleadosGrid.size() + " empleados encontrados");
    	} catch (Exception e) {
    		control(true, -1, "Ocurrio un problema al intentar consultar los Empleados", e);
    	}
    }
    ...
    public void guardarObraAlmacen() {
    	try {
    		control();
    		...
    		this.pojoObraAlmacen.setModificadoPor(this.loginManager.getUsuarioResponsabilidad().getUsuario().getId());
        	...
        	if (this.pojoObraAlmacen.getId() == null || this.pojoObraAlmacen.getId() <= 0L) {
	        	...
	        	// Guardamos 
	        	this.pojoObraAlmacen.setCreadoPor(this.loginManager.getUsuarioResponsabilidad().getUsuario().getId());
    ...
    public void eliminarObraAlmacen() {
    	try {
    		control();
	...
	public void salvarSatics() {
		try {
			...
			if (this.pojoSatics != null) {
				...
				this.pojoSatics.setModificadoPor(this.loginManager.getUsuarioResponsabilidad().getUsuario().getId());
				this.pojoSatics.setFechaModificacion(Calendar.getInstance().getTime());
				
				if (this.pojoSatics.getId() == null || this.pojoSatics.getId() <= 0L) {
					this.pojoSatics.setCreadoPor(this.loginManager.getUsuarioResponsabilidad().getUsuario().getId());
	...
	public void analizarArchivo() throws Exception {
		try {
			...
			if (! this.ifzFtp.put(this.fileSrc, this.ftpDigitalizacionRuta + this.fileName)) {
	...
	/*private void cargarCobranza() {
		...
		List<FacturaDetalleExt> lista = null;
		...
		try {
			...
			listFacturas = this.ifzFacturas.findByProperty("idObra", this.pojoObra.getId(), 120);
			if (listFacturas == null || listFacturas.isEmpty())
				return;
			
			// Recuperamos el pojo OBRA
			pojoObra = this.ifzObras.findById(this.pojoObra.getId());
			
			// Generamos el listado de cobranza
			for (Factura var : listFacturas) {
				if (var.getEstatus() == FACTURA_ESTATUS.Cancelada.ordinal())
					continue;
				
				// Recuperamos los detalles de la factura
				lista = this.ifzFacConceptos.findAllExt(var.getId());
				...
				if (itemAux != null) {
					...
					itemCobranza.setTotal(new BigDecimal(monto));
				}
	...
	public void totalizarCobranza() {
		...
		for (ObraCobranza var : this.listObraCobranza) {
			...
			if (var.getTotal().doubleValue() != monto) {
				this.permiteGuardarCobranza = false;
			}
		}
	}
	...
	public void salvarObraContrato() {
		try {
			...
			if (this.pojoObraContrato != null) {
				...
				this.pojoObraContrato.setModificadoPor(this.loginManager.getUsuarioResponsabilidad().getUsuario().getId());
				...
				if (this.pojoObraContrato.getId() == null || this.pojoObraContrato.getId() <= 0L) {
					this.pojoObraContrato.setCreadoPor(this.loginManager.getUsuarioResponsabilidad().getUsuario().getId());
	...
	public boolean getMostrarFechaEstatusContrato() {
		return (this.pojoObra != null && this.pojoObra.getEstatus() != null && "10001211".contains(this.pojoObra.getEstatusContrato().toString()));
	}

	public void setMostrarFechaEstatusContrato(boolean value) {}

	public String getFechaEstatusContrato() {
		if (this.pojoObra != null && this.pojoObra.getEstatus() != null && "10001211".contains(this.pojoObra.getEstatusContrato().toString()))
			return (new SimpleDateFormat("yyyy-MM-dd")).format(this.pojoObra.getFechaEstatus());
		return "";
	}

	public void setFechaEstatusContrato(String value) {}
	...
	public void setIdDom1(Long idDom1) {
		this.pojoDomicilioExt.setCatDomicilio1(getValorById(idDom1, this.listTiposDomicilio1).getId());
	}
	...
	public void setIdDom2(Long idDom2) {
		this.pojoDomicilioExt.setCatDomicilio2(getValorById(idDom2, this.listTiposDomicilio1).getId());
	}
	...
	public void setIdDom3(Long idDom3) {
		this.pojoDomicilioExt.setCatDomicilio3(getValorById(idDom3, this.listTiposDomicilio1).getId());
	}
	...
	public void setIdDom4(Long idDom4) {
		this.pojoDomicilioExt.setCatDomicilio4(getValorById(idDom4, this.listTiposDomicilio2).getId());
	}
	...
	public void setIdDom5(Long idDom5) {
		this.pojoDomicilioExt.setCatDomicilio5(getValorById(idDom5, listTiposDomicilio2).getId());
	}
	...
	public void setSucursal(Long idSucursal) {
		if (idSucursal == null) {
			this.pojoObra.setIdSucursal(null);
			this.pojoObra.setNombreSucursal("");
			return;
		}
		...
		for (Sucursal var : this.listSucursales) {
			if (var.getId() == idSucursal) {
				...
				this.pojoObra.setNombreSucursal(var.getSucursal());
				break;
			}
		}
	}
	...
	public boolean getIncluyeCanceladas() {
		return incluyeCanceladas;
	}

	public void setIncluyeCanceladas(boolean incluyeCanceladas) {
		this.incluyeCanceladas = incluyeCanceladas;
	}
	public boolean getMostrarTipoCambio() {
		if (this.pojoObra != null && this.pojoObra.getIdMoneda() != null && this.pojoObra.getIdMoneda() > 0L)
			return ! (this.pojoObra.getIdMoneda().longValue() == this.loginManager.getInfoSesion().getEmpresa().getMonedaId());
		return true;
	}
	...
	public double getTipoCambio() {
		if (this.pojoObra != null && this.pojoObra.getTipoCambio() > 0L)
			return this.pojoObra.getTipoCambio();
		return 1.0;
	}
	
	public void setTipoCambio(double value) {
		if (this.pojoObra != null)
			this.pojoObra.setTipoCambio(value);
	}

	public double getTipoCambioAnticipo() {
		if (this.pojoObra != null && this.pojoObra.getTipoCambioAnticipo() > 0L)
			return this.pojoObra.getTipoCambioAnticipo();
		return 1.0;
	}
	
	public void setTipoCambioAnticipo(double value) {
		if (this.pojoObra != null)
			this.pojoObra.setTipoCambioAnticipo(value);
	}

	public double getTipoCambioDeductiva() {
		if (this.pojoObra != null && this.pojoObra.getTipoCambioDeductiva() > 0L)
			return this.pojoObra.getTipoCambioDeductiva();
		return 1.0;
	}
	
	public void setTipoCambioDeductiva(double value) {
		if (this.pojoObra != null)
			this.pojoObra.setTipoCambioDeductiva(value);
	}

	public String getMontoContratadoPesos() {
		if (! getMostrarTipoCambio())
			return "";
		if (this.pojoObra.getTipoCambio() <= 0)
			this.pojoObra.setTipoCambio(1.0);
		return (new DecimalFormat("###,###,###,###.00")).format(this.pojoObra.getMontoContratado().doubleValue() * this.pojoObra.getTipoCambio());
	}

	public void setMontoContratadoPesos(String value) {}

	public String getMontoAnticipoPesos() {
		if (! getMostrarTipoCambio())
			return "";
		if (this.pojoObra.getTipoCambioAnticipo() <= 0)
			this.pojoObra.setTipoCambioAnticipo(1.0);
		return (new DecimalFormat("###,###,###,###.00")).format(this.pojoObra.getMontoAnticipo().doubleValue() * this.pojoObra.getTipoCambioAnticipo());
	}

	public void setMontoAnticipoPesos(String value) {}

	public String getMontoDeductivaPesos() {
		if (! getMostrarTipoCambio())
			return "";
		if (this.pojoObra.getTipoCambioDeductiva() <= 0)
			this.pojoObra.setTipoCambioDeductiva(1.0);
		return (new DecimalFormat("###,###,###,###.00")).format(this.pojoObra.getMontoDeductiva().doubleValue() * this.pojoObra.getTipoCambioDeductiva());
	}

	public void setMontoDeductivaPesos(String value) {}
