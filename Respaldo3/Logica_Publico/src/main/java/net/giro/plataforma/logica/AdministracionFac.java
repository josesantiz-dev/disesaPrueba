...
public class AdministracionFac implements AdministracionRem {
	...
	public Respuesta buscarPerfil(String tipoBusqueda, String valorBusqueda) {
		...
		try {
			listPerfiles = ifzPerfiles.findLikeProperty(tipoBusqueda, valorBusqueda, 0);//findLikeColumnName(tipoBusqueda, valorBusqueda);
			respuesta.getErrores().setCodigoError(0L);
	...
	@Override
	public List<Usuario> buscarUsuario(String tipoBusqueda, String valorBusqueda) throws Exception {
		try {
			return ifzUsuarios.findLikeProperty(tipoBusqueda, valorBusqueda, "", 0);//findLikeColumnName(tipoBusqueda, valorBusqueda);
		} catch (Exception e) {
			throw e;
		}
	}
	...
	@Override
	public Respuesta buscarPerfilValorUsuario(String nombrePerfil, long nivel, long nivelValor) {
		Respuesta respuesta = new Respuesta();
		List<Perfil> listPerfiles = null;
		Perfil perfil = null;
		PerfilValor perfilValor = null;
		
		try {
			listPerfiles = this.ifzPerfiles.findByColumnName("perfil", nombrePerfil);
			if (listPerfiles == null || listPerfiles.isEmpty()) {
				log.info("No se encontro el perfil " +  nombrePerfil);
				respuesta.getErrores().addCodigo(modulo, Errores.ERROR_BUSCAR_PERFILES_VALORES);
				respuesta.getErrores().setCodigoError(Errores.ERROR_BUSCAR_PERFILES_VALORES);
				respuesta.getErrores().setDescError("No se encontro el Perfil indicado: " + nombrePerfil);
				respuesta.setBody(null);
				return respuesta;
			}
			
			if (listPerfiles.size() > 1 ) {
				log.info("Mas de un perfil con el mismo nombre : "  + nombrePerfil);
				respuesta.getErrores().addCodigo(modulo, Errores.ERROR_BUSCAR_PERFILES_VALORES);
				respuesta.getErrores().setCodigoError(Errores.ERROR_BUSCAR_PERFILES_VALORES);
				respuesta.getErrores().setDescError("Mas de un perfil con el mismo nombre: " + nombrePerfil);
				return respuesta;
			}
			
			perfil = listPerfiles.get(0);
			if (perfil.getNivelPerfil() != nivel) {
				log.info("Perfil no es para nivel de Usuario: "  + nombrePerfil);
				respuesta.getErrores().addCodigo(modulo, Errores.ERROR_BUSCAR_PERFILES_VALORES);
				respuesta.getErrores().setCodigoError(Errores.ERROR_BUSCAR_PERFILES_VALORES);
				respuesta.getErrores().setDescError("Perfil no es para nivel de Usuario: " + nombrePerfil);
				return respuesta;
			}
			
			perfilValor = this.ifzPerfilValores.findByPerfilNivelValor(perfil, nivel, nivelValor);
			if (perfilValor == null || perfilValor.getId() <= 0) {
				perfilValor = new PerfilValor();
				perfilValor.setPerfil(perfil);
				perfilValor.setNivel(nivel);
				perfilValor.setValorNivel(nivelValor);
			}

			respuesta.getErrores().setCodigoError(0L);
			respuesta.getBody().addValor("perfilValor", perfilValor);
		} catch (Exception e) {
			log.error("Error en AdministracionFac.buscarPerfilValor('" + perfil + "')", e);
			respuesta.getErrores().addCodigo(modulo, Errores.ERROR_BUSCAR_PERFILES_VALORES);
			respuesta.setBody(null);
		}
	...
	@Override
	public Respuesta buscarPerfilesNivel(long nivel, long nivelValor, long idAplicacion) {
		Respuesta respuesta = new Respuesta();
		List<PerfilValor> perfilesValores = new ArrayList<PerfilValor>();
		PerfilValor perfilValor = null;
		List<Perfil> perfiles = null;
		
		try {
			perfiles = this.ifzPerfiles.findByColumnName("nivelPerfil", nivel);
			if (perfiles == null || perfiles.isEmpty()) {
				log.info("No se encontro ningun perfil del nivel indicado: " +  nivel);
				respuesta.getErrores().addCodigo(modulo, Errores.ERROR_BUSCAR_PERFILES_VALORES);
				respuesta.getErrores().setCodigoError(Errores.ERROR_BUSCAR_PERFILES_VALORES);
				respuesta.getErrores().setDescError("No se encontro ningun perfil del nivel indicado: " + nivel);
				respuesta.setBody(null);
				return respuesta;
			}
			...
			for (Perfil perfil : perfiles) {
				if (idAplicacion > 0 && perfil.getAplicacion().getId() != idAplicacion)
					continue;
				if (nivelValor > 0)
					perfilValor = this.ifzPerfilValores.findByPerfilNivelValor(perfil, perfil.getNivelPerfil(), nivelValor);
				else
					perfilValor = this.ifzPerfilValores.findPerfil(perfil, perfil.getNivelPerfil());
				if (perfilValor == null || perfilValor.getId() <= 0) {
					perfilValor = new PerfilValor();
					perfilValor.setPerfil(perfil);
					perfilValor.setNivel(nivel);
				...
				// Añadimos a listado
				perfilesValores.add(perfilValor);
			}
			
			respuesta.getErrores().setCodigoError(0L);
			respuesta.getBody().addValor("perfiles", perfilesValores);
		} catch (Exception e) {
			log.error("Error en AdministracionFac.buscarPerfilesPorNivelUsuario('" + nivel + "')", e);
			respuesta.getErrores().addCodigo(modulo, Errores.ERROR_BUSCAR_PERFILES_VALORES);
			respuesta.getErrores().setCodigoError(Errores.ERROR_BUSCAR_PERFILES_VALORES);
			respuesta.getErrores().setDescError("Ocurrio un problema al consultar los perfiles del nivel indicado: " + nivel);
			respuesta.setBody(null);
		}
	...
	@Override
	public Respuesta findUsuariosLikeProperty(String property, String value) {
		Respuesta respuesta = new Respuesta();
		try {
			List<Usuario> listUsuarios = ifzUsuarios.findLikePropiedad(property, value);
	...
	public Respuesta findEmpresasLikeClaveUsuario(String clave) {
		Respuesta respuesta = new Respuesta();
		try {
			List<Empresa> listEmpresas = ifzEmpresas.findLikeProperty("id", clave, 0);
	...
	public Respuesta findSucursalesLikeClaveUsuario(String clave) {
		Respuesta respuesta = new Respuesta();
		try {
			List<Sucursal> listSucursales = ifzSucursales.findLikePropiedad("id", clave, this.infoSesion.getEmpresa().getId());
	...
	public Respuesta  buscarUsuarioExt(String tipoBusqueda, String valorBusqueda) {
		...
		UsuarioExt usuarioExt = null;
		
		try {
			listUsuarios = ifzUsuarios.findLikeProperty(tipoBusqueda, valorBusqueda, "", 0);//findLikeColumnName(tipoBusqueda, valorBusqueda);
			for (Usuario usuario : listUsuarios) {
				try {
					usuarioExt = new UsuarioExt();
					BeanUtilsBean.getInstance().getConvertUtils().register(false, false, 0);
	...
	@Override
	public Usuario convertirUsuario(UsuarioExt target) {
		Usuario usuario = new Usuario();
		
		try {
			BeanUtilsBean.getInstance().getConvertUtils().register(false, false, 0);
			BeanUtils.copyProperties(usuario, target);			
		} catch (IllegalAccessException e) {
			e.printStackTrace();
		} catch (InvocationTargetException e) {
			e.printStackTrace();
	...
	@Override
	public UsuarioExt convertirUsuario(Usuario target) {
	...
	public Respuesta salvar(UsuarioExt usuarioExt) {
		...
		try {
			...
			respuesta.getErrores().setCodigoError(0L);
			respuesta.getBody().addValor("entity", usuario);
			respuesta.getBody().addValor("extendido", usuarioExt);
