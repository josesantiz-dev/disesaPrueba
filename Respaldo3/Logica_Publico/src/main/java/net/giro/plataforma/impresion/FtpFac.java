...
public class FtpFac implements FtpRem {
	...
	@Override
	public boolean put(byte[] fileSource, String fileName) {
	...
	@Override
	public boolean copy(String fileNameOrigen, String fileNameDestino) {
		byte[] fileSource = null;
		
		try {
			fileSource = this.get(fileNameOrigen);
			if (fileSource != null)
				return this.put(fileSource, fileNameDestino);
		} catch (Exception e) {
			log.error("Ocurrio un problema al intentar copiar el archivo indicado", e);
		}
		return false;
	}
