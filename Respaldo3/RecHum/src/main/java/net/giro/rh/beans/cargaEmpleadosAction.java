...
public class cargaEmpleadosAction implements Serializable, Runnable {
	...
	private HashMap<Long, String> listSucursales;
	// Bancos
	private BancosRem ifzBancos;
	private HashMap<Long, String> listBancos;
	// Areas
	private AreaRem ifzAreas;
	private HashMap<Long, String> listAreas;
	// Puestos
	private PuestoRem ifzPuestos;
	private HashMap<Long, String> listPuestos;
	// Categorias
	private CategoriaRem ifzCategorias;
	private HashMap<Long, String> listCategorias;
	// PuestoCategorias
	private PuestoCategoriaRem ifzPuestoCategorias;
	private HashMap<Long, String> listPuestoCategorias;
	// Periodicidad
	private ConGrupoValores pojoGpoPeriodicidad;
	private HashMap<Long, String> listPeriodicidad;
	// Formas de Pago
	private ConGrupoValores pojoGpoFormasPago;
	private HashMap<Long, String> listFormasDePago;
	// Control
	...
	private List<EmpleadoRow> listEmpleadoRow;
	private List<EmpleadoRow> listErroresRow;
	// Resultados
	...
	public cargaEmpleadosAction() {
		...
		try {
			...
			this.ifzAreas = (AreaRem) ctx.lookup("ejb:/Logica_RecHum//AreaFac!net.giro.rh.admon.logica.AreaRem");
			...
			this.ifzPuestoCategorias = (PuestoCategoriaRem) ctx.lookup("ejb:/Logica_RecHum//PuestoCategoriaFac!net.giro.rh.admon.logica.PuestoCategoriaRem");
			...
			this.ifzConGrupoValores = (ConGrupoValoresRem) ctx.lookup("ejb:/Logica_Publico//ConGrupoValoresFac!net.giro.plataforma.logica.ConGrupoValoresRem"); 
	...
	public void nuevo() {
		try {
			...
			control("Ocurrio un problema al preparar el registro", e);
		}
	}
	...
	public void carga(FileUploadEvent event) {
		try {
			...
			this.listLayout = (this.listLayout != null) ? this.listLayout : new LinkedHashMap<String, byte[]>();
			this.listLayout.put(this.fileName, this.fileSrc);
		} catch (Exception e) {
			control("Ocurrio un problema al intentar cargar el archivo indicado", e);
		}
	}
	...
	public void procesar() {
		try {
			...
			control("Ocurrio un problema al intentar procesar el archivo indicado", e);
		}
	}
	...
	public void run() {
		...
		try {
			...
			if (this.tieneRegistrosIncompletos)
				control(-1, "Carga realizada satisfactoriamente.<br>Sin embargo se encontraron " + this.listErroresRow.size() + " con informacion incompleta.");
			this.tieneRegistrosIncompletos = false;
			...
			control("Ocurrio un problema al procesar el archivo indicado en el hilo secundario", e);
		} finally {
	...
	private void doProcesar() {
		...
		try {
			...
			if(this.fileSrc == null || this.fileSrc.length <= 0) {
				...
				control(-1, "Ocurrio un problema al comprobar el archivo indicado");
				return;
			}
			...
			for (EmpleadoRow var : this.listEmpleadoRow) {
				...
				try {
					...
					bitacora("Carga fallida...", false);  
					control("Ocurrio un problema al registrar a la Persona " + codeVal, e);
					return;
				}
			}
			...
			for (EmpleadoRow var : this.listEmpleadoRow) {
				...
				try {
					...
					if (pojoEmpleado == null) {
						...
						pojoEmpleado.setIdEmpresa(getIdEmpresa());
					...
					bitacora("Carga fallida...", false);  
					control("Ocurrio un problema al registrar el Empleado " + codeVal, e);
					return;
				}
			}
			...
			for (EmpleadoRow var : this.listEmpleadoRow) {
				...
				try {
					...
					if (pojoContrato == null) {
						...
						log.info("-----> Procesando contrato " + codeVal + " :: nuevo :: " + pojoContrato.getId());
						var.setIdContrato(pojoContrato.getId());
						...
						try {
							this.ifzContratos.cancelarContratosPrevios(pojoContrato.getIdEmpleado().getId(), pojoContrato.getId(), this.loginManager.getUsuarioResponsabilidad().getUsuario().getId());
						} catch (Exception re) {
					...
					bitacora("Carga fallida...", false);  
					control("Ocurrio un problema al generar el Contrato del Empleado " + codeVal, e);
					return;
				}
			}
			...
			bitacora("Carga fallida...", false);  
			control("Ocurrio un problema al procesar el archivo indicado", e);
			dropValues();
		} 
	}
	...
	private EmpleadoContrato getContratoFromList(Long id) {
		...
		try {
			...
			log.error("Error en RecHum.cargaEmpleadosAction.getContratoFromList", e);
		}
	...
	private Empleado comparaEmpleado(Empleado pojoEmpleado, Long idPersona, EmpleadoRow pojoRegistro) {
		...
		try {
			if (actualizar) {
				pojoEmpleado.setModificadoPor(this.loginManager.getUsuarioResponsabilidad().getUsuario().getId());
				pojoEmpleado.setFechaModificacion(Calendar.getInstance().getTime());
				this.ifzEmpleados.update(pojoEmpleado);
			}
		} catch (Exception e) {
			log.error("Ocurrio un problema al intentar actualizar el Empleado " + pojoEmpleado.getId(), e);
		} finally {
	...
	private EmpleadoContrato comparaContrato(EmpleadoContrato pojoContrato, EmpleadoRow pojoRegistro) {
		...
		if (! pojoContrato.getIdEmpleado().getId().equals(pojoRegistro.getIdEmpleado()))
			return null;
		...
		if (pojoContrato.getPeriodicidadPago() != (int) getIdPeriodicidad(pojoRegistro.getPeriodicidadPago()))
			return null;
		...
		if (pojoContrato.getFormaPago() == getIdFormaDePago(pojoRegistro.getModalidadPago())) {
			actualizar = true;
		...
		try {
			...
			if (actualizar) {
				pojoContrato.setModificadoPor(this.loginManager.getUsuarioResponsabilidad().getUsuario().getId());
	...
	private Persona getPersonaFromRow(Persona pojoPersona, EmpleadoRow pojoRegistro) {
		if (pojoPersona == null) {
			...
			pojoPersona.setCreadoPor(this.loginManager.getUsuarioResponsabilidad().getUsuario().getId());
		...
		pojoPersona.setModificadoPor(this.loginManager.getUsuarioResponsabilidad().getUsuario().getId());
	...
	private Empleado getEmpleadoFromRow(Empleado pojoEmpleado, Long idPersona, String nombrePersona, EmpleadoRow pojoRegistro) {
		if (pojoEmpleado == null) {
			...
			pojoEmpleado.setCreadoPor(this.loginManager.getUsuarioResponsabilidad().getUsuario().getId());
	...
	private EmpleadoContrato getContratoFromRow(EmpleadoContrato pojoContrato, EmpleadoRow pojoRegistro) {
		if (pojoContrato == null) {
			...
			pojoContrato.setCreadoPor(this.loginManager.getUsuarioResponsabilidad().getUsuario().getId());
		...
		pojoContrato.setPeriodicidadPago(getIdPeriodicidad(pojoRegistro.getPeriodicidadPago()));
	...
	private long getIdEmpresa() {
		return this.loginManager.getInfoSesion().getEmpresa().getId();
	}
