...
public abstract class DAOImpl<T extends Serializable> implements DAO<T> {
    ...
    public long save(T entity, Long idEmpresa) throws ExcepConstraint {
		...
		try {
			...
			log.error("COMPRAS :: Error REFLECTION ", e);
		...
		try {
			...
			log.error("COMPRAS :: Error ASIGNAR SECUENCIA :: " + tabla + " :: " + secuen, e);
		...
		try {
			...
			log.error("COMPRAS :: Error GRABAR ENTIDAD :: " + tabla + " :: " + secuen, e);
		...
		for (T entity : entities) {
			...
			if (idStored <= 0) {
				try {
					...
					log.error("COMPRAS :: Error ASIGNAR SECUENCIA :: " + tabla + " :: " + secuen + " [" + index + "]", e);
					throw new ExcepConstraint("ASIGNAR SECUENCIA", entity);			
				}
				...
				try {
					...
					log.error("COMPRAS :: Error GRABAR ENTIDAD :: " + tabla + " :: " + secuen + " [" + index + "]", e);
					throw new ExcepConstraint("GRABAR ENTIDAD", entity);			
				}
			} else {
				// Actualizo
				try {
					...
					log.error("COMPRAS :: Error ACTUALIZAR ENTIDAD :: " + tabla + " :: " + idStored + " [" + index + "]", e);
