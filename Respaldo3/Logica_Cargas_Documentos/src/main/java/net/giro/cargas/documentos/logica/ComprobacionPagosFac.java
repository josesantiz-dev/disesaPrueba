...
@Stateless
public class ComprobacionPagosFac implements ComprobacionPagosRem {
	private static Logger log = Logger.getLogger(ComprobacionPagosFac.class);
	private InfoSesion infoSesion;
	// ----------------------------------------------------
	private ComprobacionPagosDAO ifzBase;

	public ComprobacionPagosFac() {
		...
		try { 
			...
			this.ifzBase = (ComprobacionPagosDAO) ctx.lookup("ejb:/Model_Cargas_Documentos//ComprobacionPagosImp!net.giro.cargas.documentos.dao.ComprobacionPagosDAO");
		} catch (Exception e) {
	...
	public ComprobacionPago save(ComprobacionPago entity) throws Exception {
		...
		try {
			if (entity != null) {
				...
				idComprobacionPago = comprobarCFDI(entity.getExpresionImpresa());
				if (idComprobacionPago <= 0L) 
					idComprobacionPago = this.ifzBase.save(entity, getCodigoEmpresa());
				entity = this.findById(idComprobacionPago);
			}
	...
	public void deleteCFDI(long idComprobacionPago) throws Exception {
		try {
			this.ifzBase.delete(idComprobacionPago);
		} catch (Exception e) {
			log.error("Ocurrio un problema en deleteCFDI(idComprobacionPago)", e);
			throw e;
		}
	}
	...
	public ComprobacionPago findById(long idComprobacionPago) throws Exception {
		try {
			return this.ifzBase.findById(idComprobacionPago);
		} catch (Exception e) {
			log.error("Ocurrio un problema en findById(idComprobacionPago)", e);
			throw e;
		}
	}
