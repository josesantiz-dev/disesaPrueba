...
public class EmpresasAction {
	...
	private void cargaMonedas(){
		...
		try {
			this.listTmpMonedas = new ArrayList<SelectItem>(); 
			respuesta = this.ifzEmpresas.cargarMonedas();
			if (respuesta.getErrores().getCodigoError() != 0L) {
				this.resOperacion = respuesta.getErrores().getDescError();
				return;
			}
			
			this.listMonedas = (List<MonedaTYGExt>) respuesta.getBody().getValor("listMonedas");
			if (this.listMonedas == null)
				this.listMonedas = new ArrayList<MonedaTYGExt>();
			
			for (MonedaTYGExt moneda : this.listMonedas)
				this.listTmpMonedas.add(new SelectItem(moneda.getId(), moneda.getNombre()));
		} catch (Exception e) {
			this.resOperacion = problemInesp;
	...
	private void asignarMonedaDescripcion(long idMoneda) {
		if (this.listMonedas == null || this.listMonedas.isEmpty())
			return;
		if (this.pojoEmpresa == null)
			return;

		this.pojoEmpresa.setMonedaId(idMoneda);
		this.pojoEmpresa.setMoneda("");
		for (MonedaTYGExt moneda : this.listMonedas) {
			if (idMoneda == moneda.getId()) {
				this.pojoEmpresa.setMoneda(moneda.getAbreviacion());
				break;
			}
		}
	}
		...
		try {
			...
			if (this.pojoEmpCertificado != null) {
				...
				if (this.pojoEmpCertificado.getPalabra() != null && ! "".equals(this.pojoEmpCertificado.getPalabra().trim())) {
					...
					this.resOperacion = "Debe indicar la contraseña del certificado";
					return;
				}
				/*
	...
	public long getIdMoneda() {
		return (this.pojoEmpresa != null ? this.pojoEmpresa.getMonedaId() : 0L);
	}
	
	public void setIdMoneda(long idMoneda) {
		asignarMonedaDescripcion(idMoneda);
	}
