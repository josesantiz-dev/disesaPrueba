...
public class PersonasAction implements Serializable {
	...
	private Logger log = Logger.getLogger(PersonasAction.class);
	...
	public PersonasAction() throws Exception{
		...
		this.problemGetFtp = propPlataforma.getString("mensajes.error.obtenerArchivoFpt");
		this.problemPutFtp = propPlataforma.getString("mensajes.error.enviarArchivoFpt");
		this.problemArchivoNec =  propPlataforma.getString("mensajes.error.archivoNecesario");
	...
	public void guardar() {
		...
		try {
			...
			if (this.pojoLocalidadNac != null)
				this.pojoPersonaExt.setLocalidad(this.pojoLocalidadNac);

			this.ifzClientes.setInfoSecion(this.loginManager.getInfoSesion());
			idPersona = this.pojoPersonaExt.getId();
			respuesta = this.ifzClientes.salvar(this.pojoPersonaExt);
			...
			if (idPersona > 0) {
				for(PersonaExt pojoPersonaAux : this.listPersonaExt) {
					if (pojoPersonaAux.getId() == idPersona) {
						this.listPersonaExt.remove(pojoPersonaAux);
						return;
					}
				}
			}
			...
			this.listPersonaExt.add(0, this.pojoPersonaExt);
		} catch (Exception e) {
	...
	private boolean validarPersona() {
		...
		if (this.pojoPersonaExt.getRfc() != null && ! "".equals(this.pojoPersonaExt.getRfc().trim())) {
			pattern = Pattern.compile(this.rfcExpresionRegular);
			matcher = pattern.matcher(this.pojoPersonaExt.getRfc());
			if (! matcher.find()) {
	...
	public void guardarEstudio() {
		try {
			ifzClientes.setInfoSecion(this.loginManager.getInfoSesion());
			this.pojoEstudioExt.setModificadoPor(loginManager.getInfoSesion().getAcceso().getUsuario().getId());
			this.pojoEstudioExt.setFechaModificacion(Calendar.getInstance().getTime());
			
			if (this.pojoEstudioExt.getId() <= 0) {
				this.pojoEstudioExt.setCreadoPor(loginManager.getInfoSesion().getAcceso().getUsuario().getId());
	...
	public void guardarDocumentoPersona() {
		...
		try {
			...
			pojoDigitalizado.setModificadoPor(loginManager.getInfoSesion().getAcceso().getUsuario().getId());
			pojoDigitalizado.setFechaModificacion(Calendar.getInstance().getTime());
			pojoDigitalizado.setCreadoPor(loginManager.getInfoSesion().getAcceso().getUsuario().getId());
	...
	public void nuevoDomicilio() throws Exception{
		this.resOperacion = "";
		try{
...
//		this.pojoDigitalizado.setDocumento(docto);
	...
	public void cargaInfoDomicilio() throws Exception{
...
//get set de los campos a desplegar
	...
	public void setPojoMunicipio(Municipio pojoMunicipio) {
		if (this.editaNacimiento) {
			this.pojoMunicipioNac = pojoMunicipio;
			this.pojoLocalidadNac = null;
		} else {
			this.pojoMunicipioDom = pojoMunicipio;
			this.pojoLocalidadDom = null;
