...
public abstract class DAOImpl<T extends Serializable> implements DAO<T> {
    ...
    public long save(T entity, Long codigoEmpresa) throws ExcepConstraint {
		Long secuen = -1L;
		Class cls = null;
		Method setter = null;
		String tabla = "";
		...
		try {
			codigoEmpresa = (codigoEmpresa != null && codigoEmpresa > 0L ? codigoEmpresa : 1L);
			codigoEmpresa = (codigoEmpresa > 10000000 ? codigoEmpresa - 10000000L : codigoEmpresa);
			if (entity == null)
				return secuen;
			...
			log.error("CXC :: Error REFLECTION ", e);
			throw new ExcepConstraint("REFLECTION", entity);
		}
		
		try {
			secuen = ifzCorrelativo.GenerarCorrelativo(codigoEmpresa, tabla);
			setter.invoke(entity, secuen);
		} catch (Exception e) {
			log.error("CXC :: Error ASIGNAR SECUENCIA :: " + tabla + " :: " + secuen, e);
			throw new ExcepConstraint("ASIGNAR SECUENCIA: " + tabla + " - " + secuen, entity);
		}
		...
		try {
			...
			log.error("CXC :: Error GRABAR ENTIDAD :: " + tabla + " :: " + secuen, e);
			throw new ExcepConstraint("GRABAR ENTIDAD: " + tabla + " - " + secuen, entity);			
		}
	...
	@Override
	@TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
	public List<T> saveOrUpdateList(List<T> entities, Long codigoEmpresa) throws Exception {
		Long secuen = -1l;
		String tabla = "";
		long idStored = 0;
		int index = 0;
		...
		for (T entity : entities) {
			...
			if (idStored <= 0) {
				try {
					...
					log.error("CXC :: Error ASIGNAR SECUENCIA :: " + tabla + " :: " + secuen + " [" + index + "]", e);
					throw new ExcepConstraint("ASIGNAR SECUENCIA: " + tabla + " - " + secuen + " [" + index + "]", entity);			
				}
				...
				try {
					...
					log.error("CXC :: Error GRABAR ENTIDAD :: " + tabla + " :: " + secuen + " [" + index + "]", e);
					throw new ExcepConstraint("GRABAR ENTIDAD: " + tabla + " - " + secuen + " [" + index + "]", entity);			
				}
			} else {
				// Actualizo
				try {
					...
					log.error("CXC :: Error ACTUALIZAR ENTIDAD :: " + tabla + " :: " + idStored + " [" + index + "]", e);
					throw new ExcepConstraint("ACTUALIZAR ENTIDAD: " + tabla + " - " + idStored + " [" + index + "]", entity);	
				}
			}
		}
    ...
    public void puedeBorrar(T entity) {
	}
