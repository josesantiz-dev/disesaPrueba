...
public class ReportesAction implements Serializable {
	...
	private String reportesUsuario;
	// VALORES DINAMICOS
	...
	public ReportesAction() {
		...
		try {
			...
			valExp = app.getExpressionFactory().createValueExpression(fc.getELContext(), "#{reportes}", SesionProc.class);
	...
	public void lanzarReporte() {
		...
		try {
			...
			reportUser = this.reportesUsuario;
			//if (! "ADMINISTRADOR JAVIITR".contains(this.usuario))
				reportUser = this.usuario;
	...
	private void cargarReportes() {
		try {
			if (this.listReportesItems == null)
				this.listReportesItems = new ArrayList<SelectItem>();
			this.listReportesItems.clear();
 	...
 	private void generarParametros() {
		...
		try {
			...
			if (this.paramsReporte == null)
				this.paramsReporte = new HashMap<String, Object>();
			this.paramsReporte.clear();
			...
			for (CParametro p : lista) {
				...
				if ("s".equals(p.getTipoEntrada())) {
					...
					input = (HtmlInputText) facesContext.getApplication().createComponent(HtmlInputText.COMPONENT_TYPE);
					...
					metExp = app.getExpressionFactory().createMethodExpression(
					...
					ajax = (HtmlAjaxSupport) facesContext.getApplication().createComponent(HtmlAjaxSupport.COMPONENT_TYPE);
					ajax.setEvent("onclick");
					ajax.setActionExpression(metExp);
					ajax.setOncomplete("busquedaDinamica('frmPrincipal:pnlBusquedaDinamica')");
					ajax.setReRender(Arrays.asList(new String[] { "frmPrincipal:pnlBusquedaDinamica" }));
					ajax.setLimitToList(true);
					
					btn = (HtmlCommandButton) facesContext.getApplication().createComponent(HtmlCommandButton.COMPONENT_TYPE);
					btn.setId("cmd" + etiqueta);
					btn.setImage("/resources/image/22/buscar.png");
					btn.setStyle("margin-left: 5px;");
					btn.getChildren().add(ajax);
					...
					calendar = (UICalendar) facesContext.getApplication().createComponent(HtmlCalendar.COMPONENT_TYPE);
					...
					combobox = (HtmlSelectOneMenu) facesContext.getApplication().createComponent(HtmlSelectOneMenu.COMPONENT_TYPE);
					...
					check = (HtmlSelectBooleanCheckbox) facesContext.getApplication().createComponent(HtmlSelectBooleanCheckbox.COMPONENT_TYPE);
