...
public abstract class DAOImpl<T extends Serializable> implements DAO<T> {
    ...
    public long save(T entity, Long idEmpresa) throws ExcepConstraint {
		...
		try {
			...
			log.error("CREDITO :: Error REFLECTION ", e);
		...
		try {
			...
			log.error("CREDITO :: Error ASIGNAR SECUENCIA :: " + tabla + " :: " + secuen, e);
		...
		try {
			...
			log.error("CREDITO :: Error GRABAR ENTIDAD :: " + tabla + " :: " + secuen, e);
		...
		for (T entity : entities) {
			...
			if (idStored <= 0) {
				try {
					...
					log.error("CREDITO :: Error ASIGNAR SECUENCIA :: " + tabla + " :: " + secuen + " [" + index + "]", e);
					throw new ExcepConstraint("ASIGNAR SECUENCIA", entity);			
				}
				...
				try {
					...
					log.error("CREDITO :: Error GRABAR ENTIDAD :: " + tabla + " :: " + secuen + " [" + index + "]", e);
					throw new ExcepConstraint("GRABAR ENTIDAD", entity);			
				}
			} else {
				// Actualizo
				try {
					...
					log.error("CREDITO :: Error ACTUALIZAR ENTIDAD :: " + tabla + " :: " + idStored + " [" + index + "]", e);
